...
def upload_images(model_name, save_path, image_dir=None, image_list=None, mode='tgt'):
    ...
    for file_path in tqdm(file_list):
        if mode == 'tgt':
            ...
            data.append({
                'index': int(file_path.stem.split('_')[3]),       # Make sure the file name's first part is the index.
                'img_path': str(file_path),
                'key': f'jarvizhang/metrics/omnicontext/{today}_{model_name}/{file_path.name}'
            })
...
if __name__ == "__main__":
    ...
    # work_dir = './__eval/editing_omnicontext'
    # for iter_model in range(200000, 240000, 1000):
    #     model_name = f'pretrain_stage3_2_anyres_iter_{iter_model:07d}'
    #     output_file = os.path.join(work_dir, model_name, "pf_sc_score.csv")
