...
from hymm.metrics.t2i_compbench.metric import T2ICompBenchMetric
from hymm.data_kits.datasets.t2i_compbench import T2ICompBenchDataset
from hymm.constants import VQA_MODEL_PATH
...
def process_img_dir(img_dir, output_base_dir=None, offset=0, 
                     csv_path="data/test/t2i_compbench.csv", 
...
使用示例:
  ...
  python calculate_t2i_compbench_offline_batch.py --img_dir "/path/to/samples/iter_0005000/t2i_compbench/images" --output_base_dir "/output" --offset 1000
    ...
    parser.add_argument(
        '--csv_path',
        type=str,
        default="data/test/t2i_compbench.csv",
        help='t2i_compbench数据集CSV文件路径，默认为 data/test/t2i_compbench.csv'
    )
