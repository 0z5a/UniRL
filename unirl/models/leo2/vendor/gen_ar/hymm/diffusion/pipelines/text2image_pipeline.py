...
class Text2ImagePipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if isinstance(prompt, str):
            ...
            raise ValueError(f"prompt should be a string or a list of strings, but got {type(prompt)}")

        prompt_inputs = text_encoder.text2tokens(prompt)
        prompt_ids = prompt_inputs["input_ids"]
        prompt_mask = prompt_inputs["attention_mask"]
        prompt_ids = prompt_ids.to(device)
        prompt_mask = prompt_mask.to(device) if self.args.text_encoder_use_attention_mask else None


        prompt_outputs = text_encoder.encode({"input_ids": prompt_ids, "attention_mask": prompt_mask})
        ...
        if prompt_attention_mask is not None:
            ...
            prompt_attention_mask = prompt_attention_mask.view(bs * num_images_per_prompt, seq_len)

        if prompt_embeds.ndim == 2:
            bs, _ = prompt_embeds.shape
        ...
        if do_classifier_free_guidance:
            ...
            negative_prompt_inputs = text_encoder.text2tokens(negative_tokens)
            negative_prompt_ids = negative_prompt_inputs["input_ids"]
            negative_prompt_mask = negative_prompt_inputs["attention_mask"]
            negative_prompt_ids = negative_prompt_ids.to(device)
            negative_prompt_mask = negative_prompt_mask.to(device) if self.args.text_encoder_use_attention_mask else None

            negative_prompt_outputs = text_encoder.encode({"input_ids": negative_prompt_ids, "attention_mask": negative_prompt_mask})
            negative_prompt_embeds = negative_prompt_outputs.hidden_state
            negative_prompt_embeds = negative_prompt_embeds.to(device=device)
            negative_prompt_attention_mask = negative_prompt_outputs.attention_mask
    ...
    def prepare_latents(self, batch_size, latent_channel, height, width, dtype, device, generator, latents=None):
        shape = (
            batch_size,
            latent_channel,
            int(height) // self.latent_scale_factor[0],
            int(width) // self.latent_scale_factor[1],
        )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            ...
            height int:
                The height of the generated sample.
            width int:
                The width of the generated sample.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.autocast(device_type="cuda", dtype=target_dtype, enabled=autocast_enabled):
                    ...
                    if hasattr(outputs, "diffusion_prediction"):
                        pred = outputs.diffusion_prediction
                    else:
                        pred = outputs['x']     # bc
