...
from .diffusion_utils import discretized_gaussian_log_likelihood, normal_kl, snr_shifter
...
def enforce_zero_terminal_snr(betas):
    ...
    sqrt_alphas_cumprod_T = sqrt_alphas_cumprod[-1]
    sqrt_alphas_cumprod -= sqrt_alphas_cumprod_T
    sqrt_alphas_cumprod *= sqrt_alphas_cumprod_0 / (sqrt_alphas_cumprod_0 - sqrt_alphas_cumprod_T)

    alphas_cumprod = sqrt_alphas_cumprod**2
    alphas = alphas_cumprod[1:] / alphas_cumprod[:-1]
    alphas = np.append(alphas_cumprod[0], alphas)
    betas = 1. - alphas
    return betas
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        alphas_cumprod = np.cumprod(alphas, axis=0)
        if shift_snr != 1.0:
            betas, alphas, alphas_cumprod = snr_shifter(alphas_cumprod, shift_snr)
            print(f"SNR shifted by {shift_snr}.")
    ...
    def q_sample(self, x_start, t, noise=None, dtype=None, device=None):
        """
        ...
        """
        if noise is None:
            noise = th.randn_like(x_start, dtype=dtype, device=device)
    ...
    def training_losses(self, model, x_start, model_kwargs=None, noise=None, timestep=None, n_tokens=None, logger=None,
                        t=None):
        """
        ...
        Args:
            ...
            x_start: the [B x C x ...] tensor of inputs.
            ...
            n_tokens: the number of tokens in the batch. Not used for ddpm. Just for API alignment.
        ...
        if model_kwargs is None:
            model_kwargs = {}
        # Time steps
        if t is None and timestep is None:
            t = th.randint(0, self.num_timesteps, (x_start.shape[0],), device=x_start.device)
        elif t is None:
            t = th.tensor([timestep] * x_start.shape[0], device=x_start.device)
        # Noise
        if noise is None:
        ...
        if logger is not None:
            logger.info(f"diffusion loss {terms['loss'].mean().item()}")
            if 'emb_loss' in extra:     
                
                logger.info(f"quantization loss: {extra['emb_loss'].mean().item()}" )
                for k, v in terms.items():
                    logger.info((f"All terms {k}: {v}"))            
            
        if 'emb_loss' in extra:     
            terms['loss'] = terms['loss'] + extra['emb_loss']
