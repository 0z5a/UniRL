...
from hymm.core.extra_model_provider import (
    build_vae,  build_text_encoder, build_scalar_state, build_tkwrapper, build_audio_vae,
)
...
_GRPO_PER_SAMPLE_LIST_KEYS = (
    # attention / RoPE structure
    "rope_media_info",
)

_GRPO_MMDIT_KWARG_KEYS = _GRPO_PER_SAMPLE_TENSOR_KEYS + _GRPO_PER_SAMPLE_LIST_KEYS


def _capture_grpo_sample_inputs(model_inputs, idx):

    def take_tensor(key):
        v = model_inputs.get(key, None)
        return v[idx].detach().clone() if isinstance(v, torch.Tensor) else None

    captured = {key: take_tensor(key) for key in _GRPO_PER_SAMPLE_TENSOR_KEYS}
...
def _stack_grpo_mini_inputs(per_sample_dicts):
    """Stack a list of per-sample GRPO input dicts into one mini-batch dict.
    """
    ...
    def _partition(key):
        ...
        if non_null and len(non_null) != mb:
            raise ValueError(
                f"_stack_grpo_mini_inputs: key {key!r} is partially None across the "
    ...
    batched = {}
    for key in _GRPO_PER_SAMPLE_TENSOR_KEYS:
    ...
    for key in _GRPO_PER_SAMPLE_LIST_KEYS:
        non_null = _partition(key)
        if not non_null:
            batched[key] = None
            continue
...
class Leo2GRPOFSDPTrainer(Leo2Trainer):
    """FSDP (pure torch) GRPO trainer for Leo."""
    ...
    def __init__(self, args):
        ...
        if args._debug_fast_init:
            ...
            args.grpo_sampling_steps = 3
            args.timesteps_group_size = 2
    ...
    def _collect_extra_client_state(self) -> dict:
        ...
        grpo_states = getattr(self, "grpo_states", None)
        if grpo_states is not None:
            state["grpo_cur_timestep"] = grpo_states.cur_timestep
            state["grpo_cur_iter_in_group"] = grpo_states.cur_iter_in_group
        ...
        state["grpo_epoch_consumed_batches"] = getattr(self, "_epoch_consumed_batches", 0)
        return state

    def _consume_extra_client_state(self, client_state: dict) -> None:
        super()._consume_extra_client_state(client_state)
        # `grpo_states` is not built yet at load time (created in train()), so stash
        # the restored values and apply them once grpo_states is constructed.
        if client_state and "grpo_cur_timestep" in client_state:
            self._resumed_grpo_state = {
                "cur_timestep": client_state.get("grpo_cur_timestep", 0),
                "cur_iter_in_group": client_state.get("grpo_cur_iter_in_group", 0),
            }
            self.logger.info(
                f"Staged GRPO states for resume: {self._resumed_grpo_state}"
            )
        ...
        if client_state and "grpo_epoch_consumed_batches" in client_state:
            self._resumed_epoch_consumed_batches = int(client_state.get("grpo_epoch_consumed_batches", 0))
            self.logger.info(
                f"Staged GRPO epoch_consumed_batches for resume: {self._resumed_epoch_consumed_batches}"
            )
    ...
    def prepare_samples_online(self, model, ref_model, reward_inferencer,
        ...
        Returns:
            ...
            all_grpo_sample_inputs: List of per-sample GRPO recompute input dicts (mmdit kwargs aligned with LeoModel.forward; see _capture_grpo_sample_inputs)
        ...
        all_prev_means = []   # Store prev_means: [Batch, Steps, ...]
        all_latents_audio = []  # Optional audio latent states: [Batch, Steps+1, ...] (empty when audio disabled)
        all_grpo_sample_inputs = []  # Per-sample GRPO recompute inputs (mmdit kwargs; see _capture_grpo_sample_inputs)
        all_videos = []       # Store generated videos: [Batch, C, F, H, W]
        all_audios = []       # Optional decoded audios: [Batch, ...] (empty when audio not decoded)
        all_condition_embeds = []  # Store condition embeddings for each sample (for training)
        ...
        for batch_start_idx in range(0, num_prompts, mini_batch_size):
            ...
            with torch.no_grad():
                ...
                pipeline_output, batch_grpo_sample_inputs, batch_condition_embeds_list = self.rollout_pipeline(
                    pipeline,
                    model,
                ...
                batch_prev_means_batch = pipeline_output.all_prev_means
                batch_log_probs_batch = pipeline_output.all_log_probs

                # Audio latents are produced for av runs and are needed for the JOINT forward during policy /
                # reference recomputation (forward conditioning is driven by use_audio_vae, NOT by
                # grpo_use_audio). Store them whenever the pipeline produced them.
                batch_latents_audio_batch = pipeline_output.all_latents_audio
                # grpo_use_audio only controls whether audio participates in the GRPO objective: when enabled,
                # fold the rollout ("old") audio log prob into the old visual log prob (the ratio denominator),
                # mirroring the joint log prob recomputed during training.
                use_audio = getattr(args, "grpo_use_audio", False)
                if use_audio and pipeline_output.all_log_probs_audio is not None:
                    batch_log_probs_batch = batch_log_probs_batch + pipeline_output.all_log_probs_audio
                # Decoded audio waveform (None unless an audio VAE + 'audio' output_type were provided).
            ...
            for local_idx in range(batch_size_actual):
                ...
                all_prev_means.append(batch_prev_means_batch[local_idx:local_idx+1]) # [1, Steps, ...]
                all_grpo_sample_inputs.append(batch_grpo_sample_inputs[local_idx])       # Dict of mmdit kwargs (LeoModel.forward style)
                all_condition_embeds.append(batch_condition_embeds_list[local_idx])   # Dict with condition embeddings
            ...
            if sp_rank == 0:
                ...
                for local_idx, (sidx, save_path) in enumerate(zip(batch_indices, batch_save_paths)):
                    ...
                    if use_latent_reward:
                        grpo_inputs_single = batch_grpo_sample_inputs[local_idx]
                        latent_single = batch_latents_batch[local_idx:local_idx+1, sample_step]
                        condition_embeds_single = batch_condition_embeds_list[local_idx]

                        if pipeline.args.extend_latent_channels:
                            channel_cond_vae_images = condition_embeds_single["channel_cond_vae_images"]
                            channel_cond_latents, channel_cond_mask, _ = pipeline.prepare_channel_cond_latents(
                                channel_cond_vae_images,
                                latent_single,
                            )
                            latent_model_input = torch.cat([latent_single, channel_cond_latents, channel_cond_mask], dim=1)
                        else:
                            latent_model_input = latent_single

                        model_kwargs = {
                            "timesteps": scheduler.timesteps[sample_step:sample_step+1],
                            "attention_mask":  grpo_inputs_single["attention_mask"],
                            "rope_media_info": grpo_inputs_single["rope_media_info"],
                            "cond_text_states": grpo_inputs_single["cond_text_states"],
                            "cond_text_mask":  grpo_inputs_single["cond_text_mask"],
                        }
                        ...
                        for k,v in model_kwargs.items():
                            model_kwargs[k] = move_to_cpu(v)
                        
                        save_path_pth = save_path.replace('.mp4', '.pth')
                        batch_pth_paths.append(os.path.abspath(save_path_pth))
                        torch.save(model_kwargs, save_path_pth)
        ...
        all_prev_means = torch.cat(all_prev_means, dim=0) # [Total_Batch, Steps, ...]
        ...
        if getattr(args, "kl_weight", 0.0) > 0 and ref_model is not None and kl_compute_mode == "rollout_phase":
            self.logger.info(f"Rank {rank}: Pre-computing Reference Model statistics (Batch-wise)...")
            ...
            # For windowed strategies (progressive/random/decay/dynamic) only the timesteps
            # in `timesteps_train` are actually trained
            ref_train_set = set(timesteps_train) if timesteps_train is not None else None
            if ref_train_set is not None:
                self.logger.info(
                    f"Rank {rank}: Reference model will only compute trained timesteps "
                    f"{sorted(ref_train_set)} (out of {num_steps}), skipping the rest."
                )

            # For av tasks the reference forward must be joint (audio+video), so prepare the audio per-step
            # timesteps too. Audio has its own flow-shift, hence its own schedule (build_audio_grpo_scheduler
            # is deterministic and matches the schedule used by the rollout / policy recomputation).
            audio_timesteps_tensor = None
            if all_latents_audio is not None:
                audio_timesteps_tensor = self.build_audio_grpo_scheduler(device).timesteps[:num_steps].to(
            ...
            for start_idx in tqdm(range(0, total_batch_size, mini_batch_size), 
                ...
                # Audio latent states for the joint reference forward (av only; None otherwise).
                mini_latents_audio = all_latents_audio[start_idx:end_idx] if all_latents_audio is not None else None
                
                # --------------------------------------------------------------------
                # 7.2.2: Stack mini-batch GRPO inputs (cfg-major layout)
                # --------------------------------------------------------------------
                # _capture_grpo_sample_inputs stored each sample's mmdit kwargs as
                # [cfg, ...] tensors; _stack_grpo_mini_inputs interleaves them into
                # [cond_0..cond_{MB-1}, uncond_0..uncond_{MB-1}] so the layout matches
                # latent_model_input = cat([latents] * cfg) inside pipeline.denoise_step.
                mini_inputs = _stack_grpo_mini_inputs(
                    all_grpo_sample_inputs[start_idx:end_idx]
                )
                # channel_cond_vae_images is NOT cfg-expanded -- just stack along sample dim.
                mini_condition_embeds_list = all_condition_embeds[start_idx:end_idx]
                if mini_condition_embeds_list[0]["channel_cond_vae_images"] is not None:
                    channel_cond_vae_images_mini = torch.stack(
                        [ce["channel_cond_vae_images"] for ce in mini_condition_embeds_list], dim=0,
                    )
                else:
                    channel_cond_vae_images_mini = None

                self.logger.info(
                    f"mini_inputs: "
                    f"cond_text_states={tuple(mini_inputs['cond_text_states'].shape)}, "
                    f"cond_text_mask={tuple(mini_inputs['cond_text_mask'].shape) if mini_inputs.get('cond_text_mask') is not None else None}, "
                    f"attention_mask={tuple(mini_inputs['attention_mask'].shape) if mini_inputs.get('attention_mask') is not None else None}, "
                    f"rope_media_info(len={len(mini_inputs['rope_media_info']) if mini_inputs.get('rope_media_info') else 0})"
                )
                
                # --------------------------------------------------------------------
                # 7.2.4: Compute Reference Model Statistics for All Timesteps
                # --------------------------------------------------------------------
                mini_batch_means = [None] * num_steps
                ref_mean_template = None
                with torch.no_grad():
                    # Process each timestep in the sampling schedule
                    for t_idx in tqdm(range(num_steps), desc="Reference model timesteps", disable=rank >= 1):
                      
                        if ref_train_set is not None and t_idx not in ref_train_set:
                            continue
                        ...
                        # For av tasks, feed the matching audio latent + audio timestep so the reference
                        # forward is joint (and the visual ref_mean is consistent with the rollout).
                        audio_latents_t = None
                        audio_timestep_t = None
                        if mini_latents_audio is not None:
                            audio_latents_t = mini_latents_audio[:, t_idx].to(device)
                            audio_timestep_t = audio_timesteps_tensor[t_idx].expand(current_batch_size)
                        ...
                        _, ref_mean, _ = self.grpo_one_step(
                            prompts,
                            ref_image_paths,
                            message_lists,
                            ...
                            channel_cond_vae_images=channel_cond_vae_images_mini,
                            audio_latents=audio_latents_t,
                            audio_timesteps=audio_timestep_t,
                            **mini_inputs,
                        )
                        
                        # Store on CPU to save GPU memory
                        ref_mean_cpu = ref_mean.cpu()
                        mini_batch_means[t_idx] = ref_mean_cpu
                        if ref_mean_template is None:
                            ref_mean_template = ref_mean_cpu

                # Fill untrained slots with zero placeholders (never read during training)
                if ref_mean_template is not None:
                    for t_idx in range(num_steps):
                        if mini_batch_means[t_idx] is None:
                            mini_batch_means[t_idx] = torch.zeros_like(ref_mean_template)
        ...
        return videos, audios, reward_scores, all_latents, all_log_probs, all_prev_means, sigma_schedule, generation_info, all_grpo_sample_inputs, all_ref_means, all_condition_embeds, ref_model_time, determistic, reward_model_load_time, reward_model_offload_time, all_latents_audio
    
    # breaks grpo_one_step into grpo_denoise_step and compute_log_prob to support pp parallel in the future
    # since only the last pp rank need to compute_log_prob
    def grpo_one_step(
        self,
        prompts,
        ref_image_paths,
        ...
        **model_input_kwargs,
        ...
        """Run one GRPO denoise step and immediately compute its log prob.

        ``model_input_kwargs`` carries the per-sample mmdit inputs captured by
        ``_capture_grpo_sample_inputs`` (cond_text_states / cond_text_mask /
        visual_mask / text_mask / audio_mask / timesteps_index /
        attention_mask / rope_media_info / und_token_indices /
        gen_token_indices / audio_token_indices), already stacked to
        ``[cfg * MB, ...]`` cfg-major layout by ``_stack_grpo_mini_inputs``.
        They are forwarded as-is into ``grpo_denoise_step``.
        """
        model_pred, scheduler, scheduler_timesteps, _audio_pack = self.grpo_denoise_step(
            prompts,
            ref_image_paths,
            message_lists,
            transformer,
        ...
        return self.compute_log_prob(scheduler_timesteps, model_pred, latents, pre_latents, scheduler, determistic)

    def grpo_denoise_step(
        self,
        prompts,
        ref_image_paths,
        message_lists,
        ...
        **model_input_kwargs,
        ...
        """Run a single denoise step of the policy / reference model.

        Builds ``model_kwargs`` from ``model_input_kwargs`` (the mmdit inputs;
        see ``_capture_grpo_sample_inputs`` for the field list and naming
        rationale -- they match ``LeoModel.forward`` 1-to-1) plus the
        GRPO-specific ``channel_cond_vae_images`` slot, then hands them to
        ``pipeline.denoise_step``. The pipeline forwards them via ``**kwargs``
        into ``LeoModelHF.prepare_inputs_for_generation`` -- if any of
        ``visual_mask`` / ``text_mask`` / ``timesteps_index`` / ``audio_mask``
        / und|gen|audio_token_indices is missing, that function raises a
        ``KeyError`` when ``use_input_ids=True`` or
        ``gen_template == "multi_stream_dit"``.

        ``None`` entries in ``model_input_kwargs`` are forwarded as ``None``
        (NOT dropped). ``LeoModelHF.prepare_inputs_for_generation`` dereferences
        these keys with ``kwargs["..."]`` rather than ``kwargs.get(...)``, so a
        missing key raises ``KeyError`` even when the model is happy to receive
        ``None`` (every relevant param on ``LeoModel.forward`` is typed
        ``Optional[...] = None``). The earlier "drop None" strategy here was
        what caused ``KeyError('timesteps_index')`` on pure-text-encoder paths
        where the capture side legitimately had nothing to fill in.
        """
        args = self.args
        transformer.train()  # ref_model 和 policy_model 都需要 train mode
        device = transformer.device
        scheduler = build_grpo_scheduler(args, sigma_schedule, device)
        ...
        if audio_latents is not None:
            # Joint visual + audio recompute. Audio has its own flow-shift / scheduler
            # / per-step timesteps (distinct from the visual ones).
            audio_latents = audio_latents.to(device)
            audio_scheduler = self.build_audio_grpo_scheduler(device)
            audio_scheduler_timesteps = audio_timesteps.to(device=device, dtype=torch.float32)
            model_pred, audio_pred, _ = pipeline.denoise_step(
            ...
            audio_pack = (audio_pred, audio_scheduler, audio_scheduler_timesteps)
        else:
            model_pred, _ = pipeline.denoise_step(
                latents, scheduler_timesteps, 0, model_kwargs, guidance_scale=guidance_scale,
            )
        return model_pred, scheduler, scheduler_timesteps, audio_pack

    def _compute_logprob_single(self, scheduler_timesteps, model_pred, latents, pre_latents, scheduler, determistic=False):
        args = self.args
        ...
        if scheduler_timesteps.dim() > 0 and scheduler_timesteps.shape[0] > 1:
            ...
            for i in range(batch_size):
                ...
                step_output = scheduler.sde_step_with_logprob(
                    single_model_pred,
                    single_latents,
                    single_timestep,
                    args.eta,
                ...
                single_latents_out, single_pred_original, single_log_prob, single_prev_mean, single_std_dev_t = step_output
            ...
            latents, pred_latents_original, log_prob, prev_mean, std_dev_t = step_output
        return log_prob, prev_mean, std_dev_t

    def compute_log_prob(self, scheduler_timesteps, model_pred, latents, pre_latents, scheduler, determistic=False,
                         audio_pack=None, audio_latents=None, audio_pre_latents=None):
        """
        Compute the per-step log probability used by the GRPO policy ratio.

        By default this is the visual log prob. The audio model prediction is still computed during the joint
        forward for av tasks (so the visual prediction is conditioned on audio), but whether the audio log
        prob actually participates in the GRPO objective is governed by ``grpo_use_audio``. When enabled (and
        ``audio_pack`` / ``audio_latents`` / ``audio_pre_latents`` are provided), the audio log prob is
        computed with the same routine and added with equal weight: ``log_prob = log_prob_visual + log_prob_audio``.

        ``prev_mean`` / ``std_dev_t`` are always the visual quantities (used by the KL / GRPO-Guard terms),
        since those tensors are modality-specific and are not combined here.
        """
        log_prob, prev_mean, std_dev_t = self._compute_logprob_single(
            scheduler_timesteps, model_pred, latents, pre_latents, scheduler, determistic,
        )
        if getattr(self.args, "grpo_use_audio", False) and audio_pack is not None and audio_latents is not None:
            audio_model_pred, audio_scheduler, audio_scheduler_timesteps = audio_pack
            if audio_model_pred is not None and audio_scheduler is not None:
                # Audio has its own flow-shift / noise schedule, so use the audio timesteps (not the visual ones).
                audio_log_prob, _audio_prev_mean, _audio_std_dev_t = self._compute_logprob_single(
                    audio_scheduler_timesteps, audio_model_pred, audio_latents, audio_pre_latents, audio_scheduler, determistic,
                )
    ...
    def train_one_step(self, model, ref_model, reward_inferencer, sp_rank, sp_group, sp_size, dp_size,
        ...
        videos, audios, reward_scores, all_latents, all_log_probs, all_prev_means, sigma_schedule, generation_info, all_grpo_sample_inputs, all_ref_means, all_condition_embeds, ref_model_time, determistic, reward_model_load_time, reward_model_offload_time, all_latents_audio = self.prepare_samples_online(
        ...
        samples = {
            ...
            "log_probs": all_log_probs,                 # [batch, num_steps] (joint when audio enabled)
            "prev_sample_mean": all_prev_means,         # [batch, num_steps, ...]
        }
        # Optional audio latents for joint (visual + audio) log-prob recomputation during training.
        if all_latents_audio is not None:
            samples["latents_audio"] = all_latents_audio[:, :-1]       # audio latent before timestep t
            samples["next_latents_audio"] = all_latents_audio[:, 1:]   # audio latent after timestep t
            # Audio has its own flow-shift, hence its own timesteps (distinct from the visual ones).
            # Take them straight from the audio scheduler so they match exactly the schedule used by the
            # log-prob recomputation (build_audio_grpo_scheduler is deterministic).
            audio_scheduler_tmp = self.build_audio_grpo_scheduler(device)
            audio_timestep_value = audio_scheduler_tmp.timesteps[:args.grpo_sampling_steps].tolist()
            audio_timestep_values = [audio_timestep_value[:] for _ in range(batch_size)]
            samples["timesteps_audio"] = torch.tensor(audio_timestep_values, device=device, dtype=torch.float32)  # [batch, num_steps]
        ...
        # Stash per-sample mmdit inputs along the sample dim. Each per-sample tensor
        # is [cfg, ...] (from _capture_grpo_sample_inputs), so the stacked tensor is
        # [B, cfg, ...]; mini_batch_iterator does the cfg-major flatten right before
        # yielding. rope_media_info is a list-of-lists with the same [B][cfg] shape.
        # Keys whose per-sample value is None across all B samples are kept as None
        # so downstream consumers (mini_batch_iterator / grpo_denoise_step) can skip
        # them; otherwise they would pollute model_kwargs with stray None entries.
        for key in _GRPO_PER_SAMPLE_TENSOR_KEYS:
            vals = [pe.get(key) for pe in all_grpo_sample_inputs]
            samples[key] = (
        ...
        for key in _GRPO_PER_SAMPLE_LIST_KEYS:
            vals = [pe.get(key) for pe in all_grpo_sample_inputs]
            samples[key] = vals if all(v is not None for v in vals) else None
        # channel_cond_vae_images is GRPO-specific (not in LeoModel.forward), not
        # cfg-expanded -- stash it untouched along sample dim.
        if all_condition_embeds[0]["channel_cond_vae_images"] is not None:
            samples["channel_cond_vae_images"] = torch.cat(
                [ce["channel_cond_vae_images"].unsqueeze(0) for ce in all_condition_embeds], dim=0,
            )
        else:
            samples["channel_cond_vae_images"] = None
        ...
        if advantages.dim() == 1:
            num_timesteps = args.grpo_sampling_steps
        ...
        enable_timestep_permutation = getattr(args, "enable_timestep_permutation", None)
        num_timesteps = len(samples["timesteps"][0])

        # Timestep permutation is incompatible with subset-based training strategies.
        # mini_batch_iterator treats each entry of train_timesteps_list (= timesteps_train,
        # the progressive window of ORIGINAL timestep indices) as a positional index into
        # samples[...][:, j]. Permuting the timestep axis breaks that identity: position j
        # would then hold a randomly chosen timestep, so the window would train the wrong
        # timesteps. Only the "all" strategy (train every position) is safe to permute.
        _train_strategy = getattr(args, "training_strategy", "all")
        if enable_timestep_permutation and _train_strategy != "all":
            raise ValueError(
                f"enable_timestep_permutation=True is incompatible with "
                f"training_strategy={_train_strategy!r}: subset/progressive strategies index "
                f"timesteps positionally, so permuting the timestep axis would train the wrong "
                f"timesteps. Set enable_timestep_permutation=False or use training_strategy='all'."
            )

        if enable_timestep_permutation:
            timestep_perms = sync_random_tensor(
            ...
            keys_to_permute = ["timesteps", "latents", "next_latents", "log_probs", "prev_sample_mean"]
            # Also permute pre-computed reference statistics if available
            if "ref_means" in samples:
                keys_to_permute.append("ref_means")
            # Also permute optional audio latents / timesteps so they stay aligned across steps.
            if "latents_audio" in samples:
                keys_to_permute.extend(["latents_audio", "next_latents_audio", "timesteps_audio"])
            for key in keys_to_permute:
                samples[key] = samples[key][batch_indices, timestep_perms]
        else:
            timestep_perms = torch.arange(num_timesteps, device=device).unsqueeze(0).expand(batch_size, num_timesteps)
        ...
        # ==================== Phase 6.5 GRPO-Guard Coefficients ====================
        use_grpo_guard = getattr(args, "use_grpo_guard", True)
        scheduler = build_grpo_scheduler(args, sigma_schedule, device)
        grpo_guard_ratio_coeffs = None
        grpo_guard_loss_coeffs = None
        if use_grpo_guard:
            with torch.no_grad():
                grpo_guard_ratio_coeffs, grpo_guard_loss_coeffs = scheduler.grpo_guard(
                    eta=args.eta,
                    device=device,
                    sde_type=args.sde_type,
                )
                logger.info(f'Rank {rank}, sp_rank {sp_rank}: grpo_guard_ratio_coeffs: {grpo_guard_ratio_coeffs}, grpo_guard_loss_coeffs: {grpo_guard_loss_coeffs}')
        ...
        # [LongCat] If using single SDE timestep selection, only train on that single timestep
        use_single_sde_timestep = getattr(args, 'use_single_sde_timestep', False)
        single_sde_timestep_idx = generation_info.get("single_sde_timestep_idx", None)
        
        if use_single_sde_timestep and single_sde_timestep_idx is not None:
            # [LongCat] Only train on the single SDE timestep for precise credit assignment
            train_timesteps_list = [single_sde_timestep_idx]
            logger.info(f"[LongCat Single SDE Timestep] Training only on timestep: {train_timesteps_list}")
        elif timesteps_train is not None:
        ...
        update_successful, grad_norm = self.forward_backward(
            ...
            info,
            mini_batch_size,
            grpo_guard_ratio_coeffs,
            grpo_guard_loss_coeffs)
    ...
    def forward_backward(
        ...
        for mini_batch in self.mini_batch_iterator(samples_batched_list, train_timesteps_list, mini_batch_size):
            with (
                ...
                loss = self.forward_step(
                    ...
                    info, mini_batch_size, grpo_guard_ratio_coeffs, grpo_guard_loss_coeffs,
                    mini_batch, model,
                )
                self.model_engine.backward(loss)

                # When mini_batch_size_per_update > 1, each inner loop processes multiple samples,
                # retain the historical sample-timestep counter for checkpoint/log compatibility.
                # Optimizer boundaries are owned exclusively by model_engine.
                ...
                # Must be called after every backward. The engine advances its micro-step
                # on non-boundaries and owns optimizer/scheduler/zero-grad/offload on boundaries.
                self.model_engine.step()
                if is_update_step:
                    update_performed = True
                    self.ss.add(update_steps=1, current_run_update_steps=1)
                    self.ss.lr = self.get_last_lr()
            ...
            if getattr(args, 'profile', False):
                if self.rank == 0:
                    ...
                    time.sleep(30)
                sys.exit(0)
        return update_performed, grad_norm

    def forward_step(
        self, sp_rank, sigma_schedule: torch.Tensor, prompts, ref_image_paths, message_lists,
        info, mini_batch_size, grpo_guard_ratio_coeffs, grpo_guard_loss_coeffs, mini_batch, model,
    ):
        # mini_batch is a dict from mini_batch_iterator. mmdit kwargs share the
        # names of LeoModel.forward (see _capture_grpo_sample_inputs); slice them
        # out and splat into grpo_denoise_step, so the trainer-side signature stays
        # 1-to-1 with the model-side signature without an "extra_kwargs" trampoline.
        mmdit_kwargs = {k: mini_batch[k] for k in _GRPO_MMDIT_KWARG_KEYS if k in mini_batch}
        model_pred, scheduler, timesteps, audio_pack = self.grpo_denoise_step(
            prompts,
            ref_image_paths,
            message_lists,
            model,
            mini_batch["latents"], # need expand
            mini_batch["timesteps"],  # need expand
            sigma_schedule,
            channel_cond_vae_images=mini_batch.get("channel_cond_vae_images"),  # need channel concat
            audio_latents=mini_batch.get("latents_audio"),  # need expand
            audio_timesteps=mini_batch.get("timesteps_audio"),  # need expand
            **mmdit_kwargs,
        )
    ...
    def grpo_loss(
        ...
        args = self.args
        # mini_batch is a dict from mini_batch_iterator. Pull the local-only fields
        # by name; mmdit kwargs (cond_text_states / cond_text_mask / visual_mask /
        # text_mask / audio_mask / timesteps_index / attention_mask / rope_media_info /
        # und|gen|audio_token_indices) stay in mini_batch and are re-splatted into
        # the fallback ref-model recompute below via _GRPO_MMDIT_KWARG_KEYS.
        latents              = mini_batch["latents"]
        pre_latents          = mini_batch["next_latents"]
        log_probs            = mini_batch["log_probs"]
        prev_sample_mean     = mini_batch["prev_sample_mean"]
        avg_advantages       = mini_batch["avg_advantages"]
        ref_means            = mini_batch.get("ref_means")
        timestep_perms       = mini_batch["timestep_perms"]
        successes            = mini_batch["successes"]
        j                    = mini_batch["j"]
        latents_audio        = mini_batch.get("latents_audio")
        next_latents_audio   = mini_batch.get("next_latents_audio")
        channel_cond_vae_images = mini_batch.get("channel_cond_vae_images")
        kl_beta = getattr(args, "kl_weight", 0.0)
        clip_range = getattr(args, "clip_range", 1e-4)
        adv_clip_max = getattr(args, "adv_clip_max", 5.0)
        reference_model_offload = getattr(args, "reference_model_offload", False)
        use_grpo_guard = getattr(args, "use_grpo_guard", True)
        # When audio is enabled, new_log_probs is the joint (visual + audio) log prob; prev_means / std_devs
        # remain visual-only (KL / GRPO-Guard are computed on the visual modality).
        new_log_probs, prev_means, std_devs = self.compute_log_prob(
            scheduler_timesteps, model_pred, latents, pre_latents, scheduler, determistic,
            audio_pack=audio_pack, audio_latents=latents_audio, audio_pre_latents=next_latents_audio,
        )
        ...
        if kl_beta > 0:
            ...
            if kl_compute_mode == "rollout_phase" and ref_means is not None:
                ...
                prev_means_ref = ref_means  # [mini_batch, ...]
            else:
                ...
                if kl_compute_mode == "training_phase":
                    ...
                    self.logger.warning(f'Rank {rank}, sp_rank {sp_rank}: Using on-the-fly KL computation (fallback mode). '
                                    f'Expected pre-computed ref_means not found.')
                with torch.no_grad():
                    # Offload models if needed to manage memory
                    if reference_model_offload:
                        self.logger.info(f'Rank {rank}, sp_rank {sp_rank}: offloading model to CPU for KL computation')
                        self.ref_model.to(device)
                        torch.cuda.empty_cache()
                    
                    _, prev_means_ref, _ = self.grpo_one_step(
                        prompts,
                        ref_image_paths,
                        message_lists,
                        self.ref_model,
                        ...
                        sigma_schedule,
                        channel_cond_vae_images=channel_cond_vae_images,
                        **{k: mini_batch[k] for k in _GRPO_MMDIT_KWARG_KEYS if k in mini_batch},
                    )
                    
                    # Restore models after reference computation
                    if reference_model_offload:
                        self.logger.info(f'Rank {rank}, sp_rank {sp_rank}: offloading ref_model to CPU after KL computation')
                        self.ref_model.to('cpu')
                        torch.cuda.empty_cache()

            # Compute KL loss if we have valid means and stds
            # Note: kl_loss_raw is per-sample, will be masked later with successes_mask
            if isinstance(prev_means, torch.Tensor) and isinstance(std_devs, torch.Tensor):
                # Align reference mean with current timestep
                # KL divergence: E[(μ - μ_ref)^2] / (2σ^2)
                if use_grpo_guard:
                    # GRPO-Guard mode: remove the std_devs ** 2 factor 
                    # to ensure similar gradient norm over different steps
                    kl_loss_raw = ((prev_means - prev_means_ref) ** 2).mean(dim=tuple(range(1, prev_means.ndim)), keepdim=False)
                else:
                    # Standard mode: use the standard KL formulation
                    kl_loss_raw = ((prev_means - prev_means_ref) ** 2).mean(dim=tuple(range(1, prev_means.ndim)), keepdim=False) / (2 * std_devs ** 2 + 1e-6)
                
                # kl_loss will be computed after successes_mask is available
        ...
        # GRPO-Guard with RatioNorm: normalizes log-importance ratio to reduce timestep-dependent variance
        # Formula: log r̂_t(θ) = guard_ratio_coeff * (log r_t(θ) + ||Δμ_θ||² / (2 * guard_ratio_coeff²))
        # where guard_ratio_coeff = σ_t * √dt (already computed in grpo_guard, std includes sqrt(-dt))
        if use_grpo_guard:
            timestep_indices = timestep_perms.to(device=self.device)
            guard_ratio_coeff = grpo_guard_ratio_coeffs[timestep_indices].to(new_log_probs.dtype)
            guard_loss_coeff = grpo_guard_loss_coeffs[timestep_indices].to(new_log_probs.dtype)
            
            # RatioNorm bias correction: ||Δμ_θ||² / (2 * guard_ratio_coeff²)
            # This shifts the mean toward zero and removes timestep-dependent variance
            ratio_mean_bias = (prev_means - prev_sample_mean).pow(2).mean(dim=tuple(range(1, prev_means.ndim)))
            ratio_mean_bias = ratio_mean_bias / (2 * guard_ratio_coeff ** 2)
            
            ratio = torch.exp((new_log_probs - log_probs + ratio_mean_bias) * guard_ratio_coeff)
        else:
            guard_ratio_coeff = 1.0
            guard_loss_coeff = 1.0
            
            ratio = torch.exp((new_log_probs - log_probs) * guard_ratio_coeff)
        ...
        policy_loss_per = torch.maximum(unclipped_loss, clipped_loss) * guard_loss_coeff
        ...
        if num_valid_samples > 0:
            ...
            self.logger.warning(f"Rank {rank}: All samples in mini-batch are filtered, using mean loss")
    ...
    def mini_batch_iterator(self, samples_batched_list, train_timesteps_list, mini_batch_size):
        """Yield per-step training mini-batches as dicts.

        Each yielded dict contains:
          * mmdit kwargs (cfg-major flattened to [cfg*MB, ...]) named after
            ``LeoModel.forward`` -- see ``_capture_grpo_sample_inputs``.
            ``forward_step`` splats these straight into ``grpo_denoise_step``.
          * GRPO-specific ``channel_cond_vae_images`` (NOT cfg-expanded).
        ...
        for _, sample in tqdm(
            ...
            # ---------------- mmdit tensor kwargs: cfg-major flatten ----------------
            # None values are kept as None (not skipped) -- see _capture_grpo_sample_inputs
            # docstring: prepare_inputs_for_generation uses kwargs["..."] not .get(...).
            mmdit_kwargs = {}
            for key in _GRPO_PER_SAMPLE_TENSOR_KEYS:
                v = sample.get(key)
                if v is None:
                    mmdit_kwargs[key] = None
                    continue
                if mini_batch_size > 1:
                    # [MB, cfg, ...] -> [cfg, MB, ...] -> [cfg*MB, ...]
                    mmdit_kwargs[key] = v.transpose(0, 1).reshape(-1, *v.shape[2:])
                else:
                    mmdit_kwargs[key] = v.squeeze(0)              # [1, cfg, ...] -> [cfg, ...]

            # ---------------- rope_media_info: cfg-major flatten of list ----------------
            rope = sample.get("rope_media_info")
            if rope is None:
                mmdit_kwargs["rope_media_info"] = None
            elif mini_batch_size > 1:
                # [[cond_0, uncond_0], ..., [cond_{MB-1}, uncond_{MB-1}]]
                # -> [cond_0, ..., cond_{MB-1}, uncond_0, ..., uncond_{MB-1}]
                mmdit_kwargs["rope_media_info"] = [
                    r for cfg_group in zip(*rope) for r in cfg_group
                ]
            else:
                mmdit_kwargs["rope_media_info"] = rope[0]

            # ---------------- GRPO-specific channel condition (no cfg expansion) ----------------
            channel_cond_vae_images = sample.get("channel_cond_vae_images")
            if channel_cond_vae_images is not None and mini_batch_size == 1:
                channel_cond_vae_images = channel_cond_vae_images.squeeze(0)
            ...
            for j in tqdm(
                ...
                mb = dict(mmdit_kwargs)
                mb["channel_cond_vae_images"] = channel_cond_vae_images
                mb["latents"]          = sample["latents"][:, j]
                mb["next_latents"]     = sample["next_latents"][:, j]
                mb["log_probs"]        = sample["log_probs"][:, j]
                mb["prev_sample_mean"] = sample["prev_sample_mean"][:, j]
                mb["avg_advantages"]   = sample["avg_advantages"][:, j]
                mb["timesteps"]        = sample["timesteps"][:, j]
                mb["timestep_perms"]   = sample["timestep_perms"][:, j]
                mb["successes"]        = sample["successes"]
                mb["j"]                = j
                mb["ref_means"]        = sample["ref_means"][:, j] if "ref_means" in sample else None
                # Optional audio latents / timesteps (None when audio disabled).
                mb["latents_audio"]      = sample["latents_audio"][:, j]      if "latents_audio"      in sample else None
                mb["next_latents_audio"] = sample["next_latents_audio"][:, j] if "next_latents_audio" in sample else None
                mb["timesteps_audio"]    = sample["timesteps_audio"][:, j]    if "timesteps_audio"    in sample else None
                yield mb
    ...
    def train(self):
        ...
        if use_single_sde_timestep_main:
            ...
            # Restore GRPO states from checkpoint if resuming. The values are staged
            # in _consume_extra_client_state() during load_after_fsdp().
            resumed_grpo_state = getattr(self, "_resumed_grpo_state", None)
            if resumed_grpo_state is not None:
                grpo_states.cur_timestep = resumed_grpo_state.get("cur_timestep", grpo_states.cur_timestep)
                grpo_states.cur_iter_in_group = resumed_grpo_state.get("cur_iter_in_group", grpo_states.cur_iter_in_group)
                logger.info(f"--> Restored GRPO states from checkpoint: cur_timestep={grpo_states.cur_timestep}, cur_iter_in_group={grpo_states.cur_iter_in_group}")
        ...
        # Expose grpo_states on the trainer so save_checkpoint()'s
        # _collect_extra_client_state() can persist its progress for resume.
        self.grpo_states = grpo_states

        # Adjust gradient accumulation steps based on training strategy
        # [LongCat] use_single_sde_timestep is checked first, independent of training_strategy
        use_single_sde_timestep = getattr(args, 'use_single_sde_timestep', False)
        if use_single_sde_timestep:
            # [LongCat] Only 1 timestep is trained, so multiply by 1
            args.gradient_accumulation_steps = args.gradient_accumulation_steps * 1 * args.video_micro_batch_size[-1]
            logger.info(f"[LongCat Single SDE Timestep] gradient_accumulation_steps adjusted for single timestep training: {args.gradient_accumulation_steps}")
        elif training_strategy in ["progressive", "random", "decay", "dynamic"]:
            # Accumulate gradients across timesteps in the group
            args.gradient_accumulation_steps = args.gradient_accumulation_steps * timesteps_group_size * args.video_micro_batch_size[-1]
        else:
            # Original behavior ("all"): accumulate across all training timesteps
            args.gradient_accumulation_steps = args.gradient_accumulation_steps * args.grpo_sampling_steps * args.video_micro_batch_size[-1]
        
        logger.info(f"gradient_accumulation_steps: {args.gradient_accumulation_steps}")

        # GRPO train_steps / args.gradient_accumulation_steps use sample-timesteps
        # as their unit, hence the video_micro_batch_size factor above. ParallelEngine
        # instead counts one micro-step per backward call, where each call processes
        # mini_batch_size_per_update samples. Convert the sample-based accumulation
        # window to the engine's micro-batch-based window without changing either
        # counter's established semantics.
        mini_batch_size = max(getattr(args, "mini_batch_size_per_update", 1), 1)
        if args.video_micro_batch_size[-1] % mini_batch_size != 0:
            raise ValueError(
                "video_micro_batch_size must be divisible by mini_batch_size_per_update "
                "for GRPO gradient accumulation"
            )
        self.model_engine.gradient_accumulation_steps = (
            args.gradient_accumulation_steps // mini_batch_size
        )
        ...
        for epoch in range(start_epoch, args.max_epochs):
            ...
            while True:
                ...
                # Update GRPO states after training step. grpo_states is the same
                # object as self.grpo_states, so its progress is persisted on
                # checkpoint via _collect_extra_client_state().
                if grpo_states is not None:
    ...
    def get_last_lr(self) -> float:
        lr = self.model_engine.get_last_lr()
    ...
    def build_audio_grpo_scheduler(self, device):
        """
        Build the audio GRPO scheduler with the audio-specific flow-shift.
        ...
        scheduler.set_timesteps(num_inference_steps=self.args.grpo_sampling_steps, device=device)
        scheduler._step_index = None
    ...
    def rollout_pipeline(
        ...
        is_av = getattr(args, "use_audio_vae", False) or self.model.generation_config.bot_task == "av"
        pipeline_kwargs = {
        ...
        # The previous `[i : i+2]` slicing was wrong: it produced overlapping windows
        # and mixed neg/pos prompts across different samples (only "worked" shape-wise
        # when len(batch_prompts) == 2 with cfg_factor == 2, but the contents were
        # already incorrect).
        B = len(batch_prompts)
        batch_grpo_sample_inputs = []
        ...
        for i in range(B):
            ...
            batch_grpo_sample_inputs.append(_capture_grpo_sample_inputs(model_inputs, idx))

            batch_condition_embeds_list.append({
                # channel condition, w/o cfg expand, only works for i2v/fi2v
                "channel_cond_vae_images": model_inputs['channel_cond_vae_images'][i:i+1].detach().clone() if model_inputs['channel_cond_vae_images'] is not None else None,
            })

        sample0 = batch_grpo_sample_inputs[0]
        self.logger.info(
            f"batch_grpo_sample_inputs.len={len(batch_grpo_sample_inputs)}, "
        ...
        # .all_log_probs / .all_prev_means and the optional audio-side .all_latents_audio / ...).
        return pipeline_output, batch_grpo_sample_inputs, batch_condition_embeds_list
