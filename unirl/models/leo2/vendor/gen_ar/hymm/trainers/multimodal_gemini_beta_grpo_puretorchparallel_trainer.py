...
class MultiModalGeminiBetaGRPOPureTorchParallelTrainer(MultiModalGeminiBetaGRPOTrainer):
    ...
    def train_step(
        ...
        def loss_closure(model_out, moe_loss=None):
            ...
            prev_sample, log_prob, prev_sample_mean, std_dev_t = sde_step_with_logprob(
                ...
                determistic=False,
                sde_noise_scale=self.args.get("sde_noise_scale", 0.7),
            )
            ...
            if self.kl_weight > 0:
                ...
                if only_kl_loss:
                    loss = kl_loss * 0.1 #0.05
            else:
                kl_loss = torch.tensor(0.0)
                loss = policy_loss
            if moe_loss is not None:
                # FIXME: 支持moe_loss_weight设置
            ...
            return loss, {
                ...
                'loss': loss.detach(),
                'unclipped_loss':unclipped_loss.detach(),
                'clipped_loss': clipped_loss.detach(),
        ...
        with torch.autocast(device_type="cuda", dtype=self.target_dtype, enabled=self.autocast_enabled):
            ...
            unclipped_loss = loss_dict['unclipped_loss']
            clipped_loss = loss_dict['clipped_loss']
