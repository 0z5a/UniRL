...
def flow_dpo_loss_fn(v_w_pred, v_l_pred, v_w_ref_pred, v_l_ref_pred, v_w_target, v_l_target, beta=500, pos_term_weight=0.0, neg_term_weight=0.0, hinge_threshold=0):
    ...
    if pos_term_weight > 0 and neg_term_weight > 0:
        ...
        neg_term = neg_term_weight * torch.clamp(ref_l_err - model_l_err, min=-hinge_threshold)  # 希望 ref_l_err 越小于 model_w_err 越好
...
class MultiModalGeminiBetaPureTorchParallelMixDPOTrainer(GeminiTrainerAlphaMultiModal):
    ...
    def task_init(self, args, all_dataset_keys=None):
        ...
        self.training_strategy = args.get("training_strategy", "all")
        self.num_timesteps = int(args.get("num_train_timesteps", 50))
        self.num_train_timesteps = int(self.num_timesteps * args.get("timestep_fraction", 1.0))
        args.diff_infer_steps = self.num_timesteps  # Make dpo training timesteps consistent with sampling timesteps
        ...
        self.timesteps_group_overlap = args.get("timesteps_group_overlap", True)
    ...
    def prepare_model_dpo_t2i_inputs(self, batch: Dict, device: Union[int, str], sample_timestep=None, **kwargs):
        ...
        return model_intput_kwargs, u_t_w, u_t_l, batch_size, n_tokens
    ...
    def train_step(self, batch, sample_timestep=None, **kwargs):
        start1 = time.time()
        model_input_kwargs, u_t_w, u_t_l, cur_batch_size, n_tokens = self.prepare_model_inputs(
            batch, 
            self.device,
        ...
        def loss_closure(model_output, moe_loss):
            ...
            loss, extra_info = flow_dpo_loss_fn(v_w_pred, v_l_pred, v_w_ref_pred, v_l_ref_pred, u_t_w, u_t_l, pos_term_weight=pos_term_weight, neg_term_weight=neg_term_weight)
    ...
    def train_loop(self):
        ...
        assert self.training_strategy != "random", "Random training strategy is not supported for DPO now."
            ...
            with profiler_context(
                ...
                for bi, batch in enumerate(self.dataloader):
                    ...
                    for timestep_idx in timesteps_train:
                        ### FIXME: 应该有问题
                        sigma = self.sampler.pipeline.scheduler.sigmas[timestep_idx:timestep_idx+1]

                        loss_dict, batch_size, n_tokens, forward_times = self.train_step(batch, sample_timestep=sigma, step=self.ss.update_steps)
                        times.update(forward_times)
                        self.logger.info(f"rank-{self.rank}, timestep-{timestep_idx}, sigma-{sigma}, loss: {loss_dict['loss']}")
