...
# =========================================================================
# Hunyuan Foundation ModelTeam
#
# Copied from deepspeed/runtime/lr_schedules.py
# Modified for extra functionality with tag `HY:`
# =========================================================================
...
def add_tuning_arguments(parser, ptm: bool = False):
    group = parser.add_argument_group('Convergence Tuning', 'Convergence tuning configurations')

    # LR scheduler
    group.add_argument('--lr-schedule', type=str, default=None, help='LR schedule for training.')

    # Learning rate range test
    group.add_argument("--lr-range-test-min-lr", type=float, default=0.001, help='Starting lr value.')
    group.add_argument("--lr-range-test-step-rate", type=float, default=1.0, help='scaling rate for LR range test.')
    group.add_argument("--lr-range-test-step-size", type=int, default=1000, help='training steps per LR change.')
    group.add_argument("--lr-range-test-staircase", type=bool, default=False, help='use staircase scaling for LR range test.')

    # OneCycle schedule
    group.add_argument("--cycle-first-step-size", type=int, default=1000, help='size of first step of 1Cycle schedule (training steps).')
    group.add_argument("--cycle-first-stair-count", type=int, default=-1, help='first stair count for 1Cycle schedule.')
    group.add_argument("--cycle-second-step-size", type=int, default=-1, help='size of second step of 1Cycle schedule (default first_step_size).')
    group.add_argument("--cycle-second-stair-count", type=int, default=-1, help='second stair count for 1Cycle schedule.')
    group.add_argument("--decay-step-size", type=int, default=1000, help='size of intervals for applying post cycle decay (training steps).')

    # 1Cycle LR
    group.add_argument("--cycle-min-lr", type=float, default=0.01, help='1Cycle LR lower bound.')
    group.add_argument("--cycle-max-lr", type=float, default=0.1, help='1Cycle LR upper bound.')
    group.add_argument("--decay-lr-rate", type=float, default=0.0, help='post cycle LR decay rate.')

    # 1Cycle Momentum
    group.add_argument('--cycle-momentum', default=False, action='store_true', help='Enable 1Cycle momentum schedule.')
    group.add_argument("--cycle-min-mom", type=float, default=0.8, help='1Cycle momentum lower bound.')
    group.add_argument("--cycle-max-mom", type=float, default=0.9, help='1Cycle momentum upper bound.')
    group.add_argument("--decay-mom-rate", type=float, default=0.0, help='post cycle momentum decay rate.')

    # Warmup LR
    group.add_argument('--warmup-min-lr', type=float, default=0, help='WarmupLR minimum/initial LR value')
    group.add_argument('--warmup-max-lr', type=float, default=0.001, help='WarmupLR maximum LR value.')
    group.add_argument('--warmup-num-steps', type=int, default=1000, help='WarmupLR step count for LR warmup.')
    group.add_argument('--warmup-type', type=str, default=WARMUP_LOG_RATE, help='WarmupLR increasing function during warmup')

    # == HY: Warmup CosineLR ==
    group.add_argument("--warmup-min-ratio", type=float, default=0.01, help='Cosine LR lower bound.')
    group.add_argument("--cos-min-ratio", type=float, default=0.01, help='Cosine LR lower bound.')
...
class WarmupCosineLR(object):
    ...
    def load_state_dict(self, sd):
        if sd is not None and 'last_batch_iteration' in sd:
            self.last_batch_iteration = sd['last_batch_iteration']
