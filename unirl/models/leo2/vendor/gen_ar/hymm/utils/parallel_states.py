...
def initialize_sequence_parallel_state(sequence_parallel_size, rank, world_size):
    global _SEQUENCE_PARALLEL_STATE
    if sequence_parallel_size > 1:
        _SEQUENCE_PARALLEL_STATE = True
        initialize_sequence_parallel_group(sequence_parallel_size, rank, world_size)
    else:
        nccl_info.sp_size = 1
        nccl_info.global_rank = rank
        nccl_info.rank_within_group = 0
        nccl_info.group_id = rank
...
def initialize_sequence_parallel_group(sequence_parallel_size, rank, world_size):
...
def initialize_teacher_student_parallel_state(sequence_parallel_size, rank, world_size):
    global _TEACHER_STUDENT_PARALLEL_STATE
