...
from hymm.samplers.instruction_tuning_multimodal_transfusion_sampler import MultimodalTransfusionSampler
...
def main():
    ...
    if args.task == "t2i":
        eval_kwargs["run_fn"] = evaluator.batch_t2i
        eval_kwargs["image_save_base"] = image_save_base

    elif args.task in ["inpainting", "editing", "subject_driven"]:
        eval_kwargs["run_fn"] = partial(evaluator.batch_ti2i, task=args.task)
        eval_kwargs["image_save_base"] = image_save_base
        ...
        eval_kwargs["run_fn"] = evaluator.batch_mmu
