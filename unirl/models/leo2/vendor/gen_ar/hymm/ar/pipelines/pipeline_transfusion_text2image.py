...
from hymm.ar.pipelines.cfg_utils import rescale_noise_cfg, ClassifierFreeGuidance, AdaptiveProjectedGuidance
from hymm.ar.pipelines.cache_utils import cache_init
...
class Text2ImageTransfusionPipeline(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        self.cfg_mode = self.args.get("cfg_mode", None)

        # Must start with APG_mode_
        if self.cfg_mode is None or self.cfg_mode == "cfg":
            self.cfg_operator = ClassifierFreeGuidance()
        elif self.cfg_mode == "apg":
            self.cfg_operator = AdaptiveProjectedGuidance(
                eta=1.0,
                adaptive_projected_guidance_rescale=10.0,
                adaptive_projected_guidance_momentum=-0.5
            )
        elif self.cfg_mode == "mixed_cfg_apg":
            self.adaptive_projected_guidance_rescale = self.args.get("adaptive_projected_guidance_rescale", 10.0)
            self.cfg_operator_apg = AdaptiveProjectedGuidance(
                eta=0.0,
                adaptive_projected_guidance_rescale=self.adaptive_projected_guidance_rescale,
                adaptive_projected_guidance_momentum=-0.5
            )
            self.cfg_operator_cfg = ClassifierFreeGuidance()
            self.apply_cfg_timesteps_ratio = self.args.get("apply_cfg_timesteps_ratio", 0.1)
        else:
            raise NotImplementedError(f"cfg_mode {self.cfg_mode} is not supported")
    ...
    @property
    def src_guidance_scale(self):
        return self._src_guidance_scale

    @property
    def do_src_classifier_free_guidance(self):
        return self._src_guidance_scale is not None and self._src_guidance_scale > 1.0
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self._src_guidance_scale = src_guidance_scale
        self._guidance_rescale = guidance_rescale

        if kwargs.get('cfg_distilled', False):
            cfg_factor = 1
            model_input_extra_kwargs["guidance"] = torch.tensor(
                [1000.0*self._guidance_scale], device=self.device, dtype=torch.bfloat16
            )
            for k, v in model_input_extra_kwargs.items():
                if v is not None and isinstance(v, (torch.Tensor, list)):
                    model_input_extra_kwargs[k] = v[:1]
        else:
            cfg_factor = 1
            if self.do_classifier_free_guidance:
                cfg_factor += 1
            if self.do_face_classifier_free_guidance:
                cfg_factor += 1
            if self.do_src_classifier_free_guidance:
                cfg_factor += 1
            if not self.do_classifier_free_guidance:
                if self.do_face_classifier_free_guidance:
                    raise NotImplementedError("Face guidance is not supported without classifier free guidance")
                if self.do_src_classifier_free_guidance:
                    raise NotImplementedError("Source guidance is not supported without classifier free guidance")
        ...
        if self.args.kv_cache:
            ...
            if not self.args.get("use_hf", False):
                self.model.set_kv_cache(batch_size=batch_size*cfg_factor,
                                        max_seq_length=T,
                                        rope_cache_length=self.model.config.get("rope_n_elem", 128), device=self.device)
        ...
        # Prepare for rope cache
        if self.use_hf:
            T = model_input_extra_kwargs["idx"].shape[1]
            self.model.set_rope_cache(
                seq_len=T, rope_image_info=model_input_extra_kwargs["rope_image_info"], device=self.device,
            )
        ...
        # taylor cache related utils
        cache_dic = None
        if self.args.use_taylor_cache:
            cache_dic = cache_init(cache_interval=self.args.taylor_cache_interval, max_order=self.args.taylor_cache_order, num_steps=len(timesteps),
                                enable_first_enhance=self.args.taylor_cache_enable_first_enhance, first_enhance_steps=self.args.taylor_cache_first_enhance_steps, 
                                enable_tailing_enhance=self.args.taylor_cache_enable_tailing_enhance,
                                tailing_enhance_steps=self.args.taylor_cache_tailing_enhance_steps,
                                low_freqs_order=self.args.taylor_cache_low_freqs_order,
                                high_freqs_order=self.args.taylor_cache_high_freqs_order)
        logger.info(f"***use_taylor_cache: {self.args.use_taylor_cache}, cache_dic: {cache_dic}")
        use_ext_model_ = should_use_external_model()
        if use_ext_model_:
            assert ext_model_forward is not None, "External model is not provided"
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.args.use_taylor_cache:
                    ...
                    model_input_extra_kwargs['cache_dic'] = cache_dic
                    ...
                    with torch.autocast(device_type="cuda", dtype=target_dtype, enabled=autocast_enabled):
                        ...
                        if not use_ext_model_:
                            ...
                            if "past_key_values" in out:
                                model_input_extra_kwargs["past_key_values"] = out["past_key_values"]

                pred = pred.to(dtype=torch.float32)

                # perform guidance
                if not kwargs.get('cfg_distilled', False):
                    if self.do_classifier_free_guidance and self.do_face_classifier_free_guidance:
                    ...
                    elif self.do_classifier_free_guidance and self.do_src_classifier_free_guidance:
                        pred_cond, pred_uncond_text, pred_uncond_text_uncond_src = pred.chunk(3)
                        pred = pred_uncond_text_uncond_src + \
                            self.guidance_scale * (pred_cond - pred_uncond_text) + \
                            self.src_guidance_scale * (pred_uncond_text - pred_uncond_text_uncond_src)

                    elif self.do_classifier_free_guidance:
                        pred_cond, pred_uncond = pred.chunk(2)
                        if self.cfg_mode == "mixed_cfg_apg":
                            if i < self.apply_cfg_timesteps_ratio * len(timesteps):
                                pred = self.cfg_operator_cfg(pred_cond, pred_uncond, self.guidance_scale, step=i)
                            else:
                                pred = self.cfg_operator_apg(pred_cond, pred_uncond, self.guidance_scale, step=i)
                        else:
        ...
        if self.args.kv_cache and not self.use_hf:
            self.model.clear_kv_cache()
        if self.use_hf:
            self.model.clear_rope_cache()
        ...
        return Text2ImageTransfusionPipelineOutput(samples=image)
