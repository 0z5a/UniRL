...
def local_get(path, size_limit=None):
...
class PatchEncodedVideo(EncodedVideo):
    """
    ...
    def from_url_or_path(
        ...
        **other_args: Dict[str, Any],
        ...
        Args:
            video_url (str): a PathManager file-path or url.
        """
        # 使用传入的视频URL
        url = video_url
        # 获取视频数据
        video_bytes, len_bytes = local_get(url, size_limit)
        if video_bytes is None:  # size exceeds limit
            raise Exception(f"Too large video file: {len_bytes} bytes, url: {video_url}")
        ...
        return video_cls(
            file=video_file,
            video_name=pathlib.Path(video_url).name,
...
def load_and_transform_video_data(
    ...
    for video_path in video_paths:
        video = PatchEncodedVideo.from_url_or_path(
            video_path,
            decoder="decord",
            decode_audio=False,
...
def load_and_transform_video_tensor_data(videos, durations, device, fps=4, clip_duration=2, clips_per_video=5):
    """
    videos: list type. Each video is a tensor in shape of [T, C, H, W]
    """
    if videos is None:
        return None
    ...
    for ind, video in enumerate(videos):
        ...
        all_clips_timepoints = get_clip_timepoints(clip_sampler, durations[ind])
