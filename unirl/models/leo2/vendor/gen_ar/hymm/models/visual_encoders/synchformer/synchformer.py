...
if __name__ == "__main__":
    ...
    synchformer.load_state_dict(
        torch.load(
            "/apdcephfs_gy2/share_302507476/1_public_models/MMAudio/ext_weights/synchformer_state_dict.pth",
            weights_only=True,
            map_location="cpu",
        )
    )

    # official synchformer ckpt, used for computing sync score
    # synchformer.load_state_dict(
    #     torch.load("/apdcephfs_gy2/share_302507476/1_public_models/MMAudio/ext_weights/synchformer_24-01-04T16-39-21.pt", weights_only=True, map_location='cpu')
    # )
    ...
    # wav =
    # features = encode_audio_with_sync(synchformer, wav, sync_mel_spectrogram).cpu()

    ### Extract video feature demo
    # vid = torch.randn(6, 1, 16, 3, 224, 224).cuda()

    # for _ in range(10):
    #     features = synchformer(vid).detach().cpu()
    ...
    #     print(f"fp32 infer cost {t1 - t0}s")
    #     print(f"fp32 mean-std:{features.mean()}, {features.std()}")

    #     with torch.autocast(device_type="cuda", enabled=True, dtype=torch.half):
    #         features2 = synchformer(vid).detach().cpu()
    #         print(f"mix-precision mean-std:{features2.mean()}, {features2.std()}")
    #     print(f"mix-pre cost {time.time() - t1}s")
    #     print((features - features2).mean())
