...
def modulate_fp32(x, shift=None, scale=None):
    if x.ndim == 3:
        shift = shift.unsqueeze(1) if shift is not None and shift.ndim == 2 else None
        scale = scale.unsqueeze(1) if scale is not None and scale.ndim == 2 else None
    input_dtype = x.dtype
    if scale is None and shift is None:
        ...
        return (x.float() * (1 + scale)).to(input_dtype)
    elif scale is None:
        return (x.float() + shift).to(input_dtype)
    else:
        return (x.float() * (1 + scale) + shift).to(input_dtype)


def apply_gate_fp32(x, gate=None, tanh=False):
    if gate is None:
        return x
    if gate.ndim == 2 and x.ndim == 3:
        gate = gate.unsqueeze(1)
