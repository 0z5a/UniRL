...
def reshape_for_broadcast(freqs_cis: Union[torch.Tensor, Tuple[torch.Tensor]], x: torch.Tensor, head_first=False):
    ...
    if isinstance(freqs_cis, tuple):
        rope_has_batch = False
        if freqs_cis[0].ndim == 3 and freqs_cis[0].shape[0] == x.shape[0]:
            rope_has_batch = True
        # freqs_cis: (cos, sin) in real space
        if head_first:
            assert freqs_cis[0].shape == (x.shape[0], x.shape[-2], x.shape[-1]) if rope_has_batch else (x.shape[-2], x.shape[-1]), f'freqs_cis shape {freqs_cis[0].shape} does not match x shape {x.shape}'
            shape = [d if i == 0 or i == ndim - 2 or i == ndim - 1 else 1 for i, d in enumerate(x.shape)] if rope_has_batch else [d if i == ndim - 2 or i == ndim - 1 else 1 for i, d in enumerate(x.shape)]
        else:
            assert freqs_cis[0].shape == (x.shape[0], x.shape[1], x.shape[-1]) if rope_has_batch else (x.shape[1], x.shape[-1]), f'freqs_cis shape {freqs_cis[0].shape} does not match x shape {x.shape}'
            shape = [d if i == 0 or i == 1 or i == ndim - 1 else 1 for i, d in enumerate(x.shape)] if rope_has_batch else [d if i == 1 or i == ndim - 1 else 1 for i, d in enumerate(x.shape)]
    ...
    if 'flash' in mode:
        ...
        if deterministic:
            if not is_flash_attn_greater_or_equal("2.4.1"):
                raise ValueError("Flash attention deterministic mode requires flash_attn>=2.4.1. "
                                 "Please upgrade flash_attn")
            flash_kwargs["deterministic"] = deterministic

        if mode == 'self_flash':
            qkv = torch.stack([q, k, v], dim=2)     # [b, s, 3, a, d]
            if attn_mask is not None:
                x = flash_attn_no_pad(qkv, attn_mask, causal=False, dropout_p=0.0, softmax_scale=None)
            else:
