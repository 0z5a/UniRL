...
class Attention(nn.Module):
    ...
    def apply_attn(self, q, k, v, causal = None, flex_attention_block_mask = None, flex_attention_score_mod = None, flash_attn_sliding_window = None):
        ...
        if flash_attn_available:
            ...
            # Always use fp16 for flash_attn to ensure consistent behavior across
            # A100 (fp16-mixed) and H20/H800 (regardless of autocast dtype).
            # bf16 has only 7-bit mantissa vs fp16's 10-bit, which degrades
            # positional encoding precision in sliding window attention.
            if fa_dtype_in != torch.float16:
            ...
            # Fallback: SDPA does not support sliding_window natively.
            # Build a band-diagonal attention mask to emulate sliding window attention.
            if flash_attn_sliding_window is not None:
                seq_len = q.shape[-2]
                window_left, window_right = flash_attn_sliding_window
                row_idx = torch.arange(seq_len, device=q.device).unsqueeze(1)
                col_idx = torch.arange(seq_len, device=q.device).unsqueeze(0)
                attn_mask = (col_idx >= row_idx - window_left) & (col_idx <= row_idx + window_right)
