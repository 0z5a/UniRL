...
def vae_sample(mean, scale, generator=None):
        stdev = nn.functional.softplus(scale) + 1e-4
        var = stdev * stdev
        logvar = torch.log(var)
        latents = torch.empty_like(mean).normal_(generator=generator) * stdev + mean
...
class VAEBottleneck(Bottleneck):
    ...
    def encode(self, x, return_info=False, generator=None, **kwargs):
        info = {}

        mean, scale = x.chunk(2, dim=1)

        x, kl = vae_sample(mean, scale, generator=generator)
...
class RVQVAEBottleneck(DiscreteBottleneck):
    ...
    def encode(self, x, return_info=False, generator=None):
        info = {}

        x, kl = vae_sample(*x.chunk(2, dim=1), generator=generator)
...
class DACRVQVAEBottleneck(DiscreteBottleneck):
    ...
    def encode(self, x, return_info=False, n_quantizers: int = None, generator=None):
        info = {}
