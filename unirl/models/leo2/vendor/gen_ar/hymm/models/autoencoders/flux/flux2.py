...
class AutoEncoder(nn.Module):
    ...
    def from_pretrained(cls, path: str):
        """
        从预训练路径加载模型和配置, 会自动查找目录下的config.json文件
        """
        path = Path(path)

        # 优先查找config.json，如果不存在则使用默认配置
        ...
        # 加载模型权重
        model.load_state_dict(torch.load(os.path.join(path, "model.pt"), map_location="cpu", weights_only=True), strict=True)
        return model
