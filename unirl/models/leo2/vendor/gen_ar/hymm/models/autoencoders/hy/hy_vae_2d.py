# copied from https://git.woa.com/text2video/hunyuan_text2video_stdit_ptm/commit/7c618aef928136021b69cac95a6ac9a7d5003a4b
...
class Conv3d(nn.Conv3d):
    ...
    def __init__(self, in_channels: int, out_channels, kernel_size, stride=1, padding=0, *args, **kwargs):
        # 2D
        ...
        super().__init__(in_channels, out_channels, kernel_size=kernel_size, stride=stride, padding=padding, *args, **kwargs)
...
class Downsample(nn.Module):
    def __init__(self, in_channels: int, out_channels, add_temporal_downsample: bool = True):
        super().__init__()
        self.add_temporal_downsample = add_temporal_downsample
        stride = (2, 2, 2) if add_temporal_downsample else (1, 2, 2)  # THW
        # no asymmetric padding in torch conv, must do it ourselves
        self.conv = Conv3d(in_channels, out_channels, kernel_size=3, stride=stride, padding=0)
    ...
    def forward(self, x: Tensor):
        ...
        if x.shape[2] == 1:
            temporal_pad = (0, 0, 0, 0, 0, 0)
        else:
            if self.add_temporal_downsample:
                temporal_pad = (0, 0, 0, 0, 0, 1)
            else:
                temporal_pad = (0, 0, 0, 0, 1, 1)
...
class Upsample(nn.Module):
    def __init__(self, in_channels: int, out_channels, add_temporal_upsample: bool = True):
        super().__init__()
...
class HYVAE2D(ModelMixin, ConfigMixin):
