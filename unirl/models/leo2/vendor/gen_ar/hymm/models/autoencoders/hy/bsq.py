...
class BinarySphericalQuantizer(nn.Module):
    def __init__(
        ...
        if embed_dim > 32:
            raise Exception("The codebook size is too large. Try use other codebook")
        ...
        loguru.logger.warning(f"BSQ {self.l2_norm=}")
        self.inv_temperature = inv_temperature

        self.register_buffer("basis", 2 ** torch.arange(embed_dim - 1, -1, -1, dtype=torch.long), persistent=False)
    ...
    def quantize(self, z):
        ...
        zhat = torch.where(
            z > 0, torch.tensor(1, dtype=z.dtype, device=z.device), torch.tensor(-1, dtype=z.dtype, device=z.device)
        )
    ...
    def forward(self, z):
        ...
        if self.input_format == "bchw":
            ...
            zq = rearrange(zq, "(b t) h w c -> b c t h w", b=original_b)
            indices = rearrange(indices, "(b t) h w -> b t h w", b=original_b)
            # group_indices = rearrange(group_indices, '(b t) h w -> b t h w', b=original_b)
        else:
            raise ValueError(f"Invalid input_format {self.input_format}")

        # beta=0., gamma0=1.0, gamma=1.0, zeta=1.0, # 函数默认参数
        ...
        return (
            ...
            commit_loss + self.zeta * entropy_penalty / self.inv_temperature,
            {
                ...
                "avg_prob": avg_prob,
                # 'raw_commit_loss': torch.mean(((zq.detach() - z) ** 2).sum(dim=-1)),
                # 'raw_entropy_penalty': entropy_penalty / self.inv_temperature
            },
        )
    ...
    def soft_entropy_loss(self, z):
        ...
        group_code_book = self.group_codebook / (self.embed_dim**0.5 if self.l2_norm else 1)  # [2**1, 1]
        divided_z = rearrange(z, "... (g c) -> ... g c", c=self.group_size)  # [1, 32, 32, 18, 1] # [b h w c/g g]
    ...
    def codes_to_indexes(self, zhat):
        ...
        return ((zhat.to(torch.int64) + 1) / 2 * self.basis).sum(axis=-1).to(torch.int64)
    ...
    def get_group_codebook_entry(self, group_indices):
        raise RuntimeError("Unchecked code")
...
LossBreakdown = namedtuple(
    "LossBreakdown", ["per_sample_entropy", "codebook_entropy", "commitment", "avg_probs", "extra_weighted_loss"]
)


class BSQWrapper(BinarySphericalQuantizer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.zero = torch.tensor(0.0)

    def forward(self, h, return_loss_breakdown=True):
        quant, loss, log = super().forward(h)
        # LFQ 返回 (quantized, entropy_aux_loss, indices), loss_break
        # "indices": indices, "group_indices": group_indices,
        ret = (quant, self.zero, log["indices"])
        if not return_loss_breakdown:
            return ret
        return ret, LossBreakdown(self.zero, self.zero, self.zero, self.zero, loss)
