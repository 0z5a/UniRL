...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        ...
        if self.xattn:
            ...
            #################### Flash attention
            '''
            Code template for scaled_dot_product_attention
            ...
            x = torch.nn.functional.scaled_dot_product_attention(
                q, k, v, dropout_p=self.xattn_drop, scale=self.scale, is_causal=False
            )

            x = x.transpose(1, 2) # y.shape=(B, nh_v, T, hs) -> y.shape=(B, T, nh_v, hs)
            x = x.reshape(B, N, -1)
            #################### Naive attention
