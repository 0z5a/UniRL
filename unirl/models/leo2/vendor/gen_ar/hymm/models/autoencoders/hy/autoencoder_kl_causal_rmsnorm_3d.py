...
from hymm.models.autoencoders.hy.autoencoder_kl_3d import (
...
class CausalConv3d(nn.Module):
    def __init__(
        ...
        if disable_causal:
            padding = (kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2,
                       kernel_size // 2)
        else:
            padding = (kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size // 2, kernel_size - 1,
                       0)  # W, H, T
...
class AutoencoderKLConv3D(ModelMixin, ConfigMixin):
    ...
    def encode(self, x: Tensor, return_dict: bool = True):
        ...
        if len(x.shape) != 5:  # (B, C, T, H, W)
            x = x.unsqueeze(2)
        assert len(x.shape) == 5, f"Wrong x shape: {x.shape}"  # (B, C, T, H, W)
        # if x.shape[2] == 1:
