...
def l2norm(t, dim = -1,  eps = 1e-6):
    t_dtype = t.dtype
    t = F.normalize(t, p = 2, dim = dim, eps = eps)
    return t.to(t_dtype)
...
def cdist(x, y, chunk_size=1024):
    """Memory-efficient implementation of cdist using chunking
    Args:
        x: tensor of shape [b, n, d]
        y: tensor of shape [b, m, d]
        chunk_size: size of chunks to process at once; split x into chunks of size chunk_size
    Returns:
    ...
    device = x.device
    type = x.dtype
    # Pre-compute y2 since it's used for all chunks
    y2 = reduce(y ** 2, 'b n d -> b n', 'sum')
    
    # Initialize output tensor
    distances = torch.zeros(b, n, m, device=device, dtype=type)
    
    # Process x in chunks
    print(f"x shape: {x.shape}, n: {n}, chunk_size: {chunk_size}")
    for i in range(0, n, chunk_size):
        end_i = min(i + chunk_size, n)
        # Get current chunk
        x_chunk = x[:, i:end_i]
        
        # Compute x2 for chunk
        x2_chunk = reduce(x_chunk ** 2, 'b n d -> b n', 'sum')
        
        # Compute xy for chunk
        xy_chunk = einsum('b i d, b j d -> b i j', x_chunk, y)
        
        # Compute distances for chunk
        distances[:, i:end_i] = (
            rearrange(x2_chunk, 'b i -> b i 1') + 
            rearrange(y2, 'b j -> b 1 j') + 
            xy_chunk * -2
        ).clamp(min=0).sqrt()
...
def gumbel_sample(
    ...
    # F.one_hot is a very large matrics, clear cache after it is used; not sure whether it can make inference slower
...
def batched_sample_vectors_old(samples, num):
    return torch.stack([sample_vectors(sample, num) for sample in samples.unbind(dim = 0)], dim = 0)

def batched_sample_vectors(samples, num):
    """
    Sample 'num' vectors from each batch of samples independently.
    
    Args:
        samples: Tensor of shape [batch_size, *dimensions] 
                Each batch element contains a set of vectors to sample from
        num: Integer, number of vectors to sample from each batch element
    
    Returns:
        Tensor of shape [batch_size, num, *vector_dims] containing sampled vectors
    
    Example:
        If samples has shape [4, 1000, 256] (batch of 4, each with 1000 vectors of dim 256)
        and num=100, output will have shape [4, 100, 256] 
        (batch of 4, each with 100 randomly sampled vectors)
    """
    # 1. unbind splits the tensor along batch dimension (dim=0) into a tuple of tensors
    batch_tensors = samples.unbind(dim=0)
    
    # 2. Initialize empty list to store sampled vectors for each batch
    sampled_batch = []
    
    # 3. Process each batch element independently
    for single_batch in batch_tensors:
        # single_batch = batch_tensors[0]
        # sample_vectors randomly selects 'num' vectors from single_batch
        # If single_batch shape is [1000, 256], and num=100
        # sampled result will be [100, 256]
        sampled_vectors = sample_vectors(single_batch, num)
        
        # Add the sampled vectors for this batch to our list
        sampled_batch.append(sampled_vectors)
    
    # 4. Stack the sampled vectors back together along batch dimension
    # If we had batch_size=4, final shape will be [4, 100, 256]
    return_stack =  torch.stack(sampled_batch, dim=0)
    return return_stack
...
def kmeans(
    ...
    """
    kmeans clustering for vector quantization
    First, sample num_clusters vectors from samples,   samples.shape=torch.Size([1, 128, 256]) -> means.shape=torch.Size([1, 16384, 256])
    Then for num_iters 10 iterations , update samples batch size 128 of vectors using kmeans()
    """

    num_codebooks, dim, dtype, device = samples.shape[0], samples.shape[-1], samples.dtype, samples.device

    means = sample_fn(samples, num_clusters)
    # FIXME merge with log_in_safe_logger
    def log_tensor_stats(name, tensor):
        return f"{name}: shape={tensor.shape}, range=[{tensor.min().item():.4f}, {tensor.max().item():.4f}], μ={tensor.mean().item():.4f}, σ={tensor.std().item():.4f}"
    if logger is not None:
        logger.info("--------------------------------kmeans start--------------------------------")
        logger.info(log_tensor_stats("samples", samples))
        logger.info(log_tensor_stats("means[0,:10]", means[0,:10]))
        logger.info(log_tensor_stats("means[-1,-10:]", means[-1,-10:]))
        logger.info(log_tensor_stats("means (full)", means))
    
    for _ in range(num_iters):
        last_means = means.clone()
        torch.cuda.empty_cache()
        with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16):
            if use_cosine_sim: 
                dists = samples @ rearrange(means, 'h n d -> h d n')
            else:
                dists = -cdist(samples.to(torch.bfloat16), means.to(torch.bfloat16))
        torch.cuda.empty_cache()
        ...
        if _ % 10 == 0 and logger is not None:
            # calculate relative update in means
            value_differece = ((means - last_means)**2).sum() / (means**2).sum()
            sqrt_difference = value_differece.sqrt()
            # extract maximum distances for chosen buckets
            buckets_expanded = buckets.unsqueeze(-1)  # shape: [1, 115200, 1]
            dists_max = torch.gather(dists, dim=2, index=buckets_expanded)  # shape: [1, 115200, 1]
            dists_max = dists_max.squeeze(-1)  # shape: [1, 115200]
            # convert torch int tensor to float tensor
            bins_float = bins.clone().detach().float()
            logger.info(f"\n In iteration {_}, sqrt value difference in means: {sqrt_difference:.6f}")
            logger.info(log_tensor_stats("Distance for all input tokens and all codes", dists))
            logger.info(log_tensor_stats(f"Theoretical mean {samples.shape[1]/num_clusters}; Cluter size of each codebook", bins_float))
            logger.info(log_tensor_stats("Cosine similarity for input tokens and closest code", dists_max))
            logger.info(log_tensor_stats("Cosine similarity for input tokens and closest code (normalized)", dists_max/dists.max()))

    if logger is not None:      
        logger.info("--------------------------------kmeans end--------------------------------")
        logger.info(log_tensor_stats("means[0,:10]", means[0,:10]))
        logger.info(log_tensor_stats("means[-1,-10:]", means[-1,-10:]))
        logger.info(log_tensor_stats("means (full)", means))

    
    return means.to(torch.float32), bins
...
class EuclideanCodebook(Module):
    def __init__(
        ...
        # Tracks how many times each codebook vector is used
        self.register_buffer('cluster_size', torch.ones(num_codebooks, codebook_size))
        # Maintains a moving average of the embeddings assigned to each codebook vector
...
# FIXME, current implementation have too much layer of quantizer
class CosineSimCodebook(Module):
    def __init__(
        ...
        if self.pretrained_path is not None:
            if logger is not None:
                logger.info(f"Loading buffers from pretrained path: {self.pretrained_path}")
            self.load_buffers_from_pretrained(self.pretrained_path)
        
        if logger is not None:
            logger.info("--------------------------------Buffers in CosineSimCodebook--------------------------------")
            for name, buffer in self.named_buffers():
                log_in_safe_logger(buffer, logger, f"{name}: ")

            logger.info("--------------------------------Parameters in CosineSimCodebook--------------------------------")
            for name, parameter in self.named_parameters():
                log_in_safe_logger(parameter, logger, f"{name}: ")

    def load_buffers_from_pretrained(self, pretrained_path):
        pretrained_state_dict = torch.load(pretrained_path, map_location=torch.device('cpu'))
        log_in_safe_logger(pretrained_state_dict, self.logger, f"Initialization of CosineSimCodebook pretrained_state_dict: ")
        
        missing, unexpected = self.load_state_dict(pretrained_state_dict, strict=False)
        self.logger.info(f"Restored CosinSimCodebook with {len(missing)} missing and {len(unexpected)} unexpected keys")
        if len(missing) > 0:
            self.logger.info(f"Missing Keys: {missing}")
            self.logger.info(f"Unexpected Keys: {unexpected}")

    @torch.jit.ignore
    def init_embed_(self, data, mask = None, logger=None):
        if self.initted:
            return
    ...
    def replace(self, batch_samples, batch_mask):
        """
        Replace the codes in the codebook with the new codes from the input data stream
        
        Args:
            batch_samples: the input data stream
            batch_mask: the mask of the input data stream
        """
    ...
    def expire_codes_(self, batch_samples):
        """
        Remove the codes in the codebook that have not been used in a long time
        ...
        batch_samples = rearrange(batch_samples, 'h ... d -> h (...) d')
        print(f"Replace expired codes: {expired_codes.sum()}")
        self.replace(batch_samples, batch_mask = expired_codes)
    ...
    def forward(
        ...
        # The codebook self.embed is bf16, 
        # but the input x can be fp16, 
        # so we should convert x to type of self.embed
        input_dtype = x.dtype
        x = x.to(self.embed.dtype)
        needs_codebook_dim = x.ndim < 4
        sample_codebook_temp = default(sample_codebook_temp, self.sample_codebook_temp)

        # dangerous, we should leave flexibility 
        # to use bf16 using torch autocast 
        # or devide outside this forward
        # x = x.float()
        ...
        if self.training and self.ema_update and not freeze_codebook:
            print("WARNING: EMA update is used in Codebook, it will change the codebook")
            if exists(mask):
                embed_onehot[~mask] = 0.

            bins = embed_onehot.sum(dim = 1)
            self.all_reduce_fn(bins)
            # Keep tracking the cluster size in the input data stream
            ema_inplace(self.cluster_size.data, bins, self.decay) 
            # Add the input data stream to corresponding code bin indices
            embed_sum = einsum('h n d, h n c -> h c d', flatten, embed_onehot)
            embed_sum = embed_sum.contiguous()
            self.all_reduce_fn(embed_sum)
        ...
        dist = unpack_one(dist, 'h * d')
        return quantize.to(input_dtype), embed_ind, dist
    

class CosineSimVQCodebook(Module):
    def __init__(
        self,
        dim,
        codebook_size,
        ...
        logger=None,
        pretrained_path=None,
        # hidden_dim = 65536, # to large, has OOM during backward 
        # hidden_dim = 1024*4,
        # hidden_scale = 1.0
        # hidden_scale = 10 # increase gradients for debug
        ...
        self.register_buffer('frozen_embed', embed) # FIXME, we want learnable_codebook
        def init_zero_linear(dim_in, dim_out):
            layer = nn.Linear(dim_in, dim_out)
            nn.init.zeros_(layer.weight)
            nn.init.zeros_(layer.bias)
            return layer
        
        self.code_transform = init_zero_linear(dim, dim)
        self.logger = logger
        ...
        if self.logger is not None: 
            for name, param in self.code_transform.named_parameters():
                safe_check_grad(f"code_transform.{name}", param, self.logger)

    @property
    def embed(self):
        return self.code_transform(self.frozen_embed) + self.frozen_embed    
    ...
    def load_buffers_from_pretrained(self, pretrained_path):
        ...
        if len(missing) > 0:
            self.logger.info(f"Missing Keys: {missing}")
            self.logger.info(f"Unexpected Keys: {unexpected}")
        # log_in_safe_logger(self.embed, self.logger, f"Restored CosinSimCodebook embed: ")
        self.logger.info(f"Keys in pretrained_state_dict: {pretrained_state_dict.keys()}")
        if 'embed' in pretrained_state_dict:
            self.frozen_embed.data.copy_(pretrained_state_dict['embed'])
            log_in_safe_logger(self.frozen_embed, self.logger, f"Restored CosinSimCodebook frozen_embed from embed: ")
    ...
    def init_embed_(self, data, mask = None, logger=None):
        ...
        self.frozen_embed.data.copy_(embed)
        self.embed_avg.data.copy_(embed_sum)
        self.cluster_size.data.copy_(cluster_size)
        self.initted.data.copy_(torch.Tensor([True]))
...
class VectorQuantize(Module):
    def __init__(
        ...
        if use_cosine_sim == False:
            codebook_class = EuclideanCodebook
        elif use_cosine_sim == True:
            codebook_class = CosineSimCodebook
        elif use_cosine_sim == 'simvq':
            codebook_class = CosineSimVQCodebook
        ...
        codebook_kwargs = dict(
            ...
            pretrained_path = codebook_ckpt_path,
            logger = logger,
        ...
        log_in_safe_logger(codebook_kwargs, logger, f"Initialize {codebook_class} using kwargs:")
