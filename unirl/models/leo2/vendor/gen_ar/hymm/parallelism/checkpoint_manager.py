...
from .stateful import OptimizersContainer, LRSchedulersContainer, ModelWrapper, EPModelWrapper, wrap_model_to_stateful
...
def save_with_gc(state, checkpoint_id):
    ...
    # if device_mesh['dp_replicate'].get_local_rank()== 0:
    ...
    #     if parallel_dims.dp_replicate_mesh.get_local_rank()== 0:
    #         dcp.save(state, checkpoint_id=checkpoint_id, process_group=parallel_dims.world_mesh['all_shard'].get_group())
...
class CheckpointManager:
    ...
    def __init__(
        ...
        if model_parts is not None:
            self.states.update(
                {
                    MODEL: wrap_model_to_stateful(model_parts),
                    # DATALOADER: dataloader,
    ...
    @torch.no_grad()
    def save(self, step_or_tag: Union[int, str], force: bool = True) -> None:
        ...
        if force:
            self._save_last_step(step_or_tag)
    ...
    @torch.no_grad()
    def load_from_path(self, path, strict=False) -> bool:
        checkpoint_id = path
        if not os.path.isdir(checkpoint_id):
            raise ValueError(f"Checkpoint path {checkpoint_id} should be a valid directory.")

        logger.info(f"Loading the checkpoint from {path}.")
        begin = time.monotonic()
        states = self._states_to_load(path)
        torch.cuda.empty_cache()
    ...
    def _create_checkpoint_id(self, step_or_tag: Union[int, str], folder: str = "") -> str:
        ...
        if isinstance(step_or_tag, str):
            ...
            return os.path.join(folder, f"step-{step_or_tag}/{self.ckpt_config.folder}")
        else:
    ...
    def _save_last_step(self, step_or_tag: Union[int, str]) -> None:
        ...
        save_with_gc(self.states, checkpoint_id=self._create_checkpoint_id(step_or_tag))
    ...
    if sd_input:
        ...
        try:
            ...
            if '`weights_only` argument in `torch.load` from `False` to ' in str(e):
                sd = torch.load(sd_path, weights_only=False, map_location='cpu')
    while 'model' in sd:
        sd = sd['model']
    while 'module' in sd:
        sd = sd['module']
