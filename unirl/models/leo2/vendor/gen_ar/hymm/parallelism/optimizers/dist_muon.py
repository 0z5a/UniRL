# PTM Muon Optimizer
# This code file implemented the distributed version of MUON to parallize the N-S iterations of different parameters
# Author: neowywang
            ...
            for base_i in range(len(params))[::dist_size]:
                ...
                for i, p in enumerate(params_pad[base_i: base_i+dist_size]):
                        if isinstance(p.grad, DTensor):
                            ...
                            # if g_buf[-1].ndim > 2:
                            #     g_buf[-1] = g_buf[-1].view(g_buf[-1].size(0), -1)
                        else:
                ...
                if base_i+local_rank < len(params):
                    ...
                    g_buf[local_rank] = g.view(p.shape)
...
def get_dist_muon(
        params,
        lr=1e-3, weight_decay=0.1, momentum=0.95, adamw_betas=(0.95, 0.95), adamw_eps=1e-8
):
    params = list(params)
    muon_params = [
        p
    ...
    if parallel_dims.world_size // parallel_dims.pp > 8:
        assert (parallel_dims.world_size // parallel_dims.pp) % 8 == 0
        dist_size = 8
    else:
        dist_size = parallel_dims.world_size // parallel_dims.pp
    return Dist_Muon(
        lr=lr,
        wd=weight_decay,
