# =======================================================
# Caption process script used for caption_v4_zh and caption_v4_en
# =======================================================
...
def is_none_str(text):
    return text.strip().lower() in NONE_STRS["en"] or text.strip() in NONE_STRS["zh"]
...
class CaptionAug:
    def __init__(self,
        ...
        self.tag_keys = {"background", "shot_type", "style", "light", "atmosphere", "composition", "color_tone", "IP"}
    ...
    def apply_tag_strategy(self, tag_candidates, lang, caption, caption_key):
        ...
        # long_caption 和 long_long_caption 自带除了 style 和 IP 之外的 tag
        if caption_key in ["long_caption", "long_long_caption"]:
            tag_candidates = {k: v for k, v in tag_candidates.items() if k in ["style", "IP"]}
    ...
    def random_compose(self, caption_dict, lang, return_key=False, caption_keys=None, tag_keys=None,
        ...
        for key in caption_dict:
            if key in valid_caption_keys:
                if caption_dict[key] and caption_dict[key] != "" and not is_none_str(caption_dict[key]):
                    caption_key_candidates.append(key)
            elif key in valid_tag_keys:
                if ignore_tag_prob or random.random() < self.caption_sample_ratio[key]:
                    tag = caption_dict.get(key, "")
                    if tag and tag != "" and not is_none_str(tag):
        ...
        if len(tag_candidates) > 0:
            tag_candidates_lst, tag_candidates = self.apply_tag_strategy(tag_candidates, lang, caption, selected_key)
