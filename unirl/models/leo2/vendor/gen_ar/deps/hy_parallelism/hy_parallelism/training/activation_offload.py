...
from torch.distributed.algorithms._checkpoint.checkpoint_wrapper import OffloadWrapper, ActivationWrapper
# import torchao
...
class OffloadActivations(saved_tensors_hooks):
    ...
    def __init__(
            ...
            if self.is_first_forward_call:
                if len(self.tracker) > 0:
                    loguru.logger.warning(
                        f'When performing forward call, {len(self.tracker)} elements left in tracker detected.  '
                        'This indicates that the backward pass is not called after the forward pass or '
                        'not all activations are unpacked. This is not expected and may cause memory leak. '
                        'Here we clear the graph node activations and continue. '
                    )
                    self.clear()
                # assert (
                #     len(self.tracker) == 0
                # ), f"backward pass should have cleared tracker of all tensors. {len(self.tracker)} elements left"
        ...
        def unpack_tensor_with_streams(unpack_tensor_id: int) -> torch.Tensor:
            ...
            # clear tensor from tracking
            # print(f'when clear tracker, {len(self.tracker)} elements left')
            del self.tracker[unpack_tensor_id]
            return maybe_gpu_tensor
...
@cache
def get_activation_offload_context(*args, **kwargs):
    return OffloadActivations(*args, **kwargs)


class OffloadWrapperNoPin(ActivationWrapper):
...
def offload_wrapper_nopin(module: torch.nn.Module) -> torch.nn.Module:
    return OffloadWrapperNoPin(module)

def compose_saved_tensor_hooks(outer, inner):
    outer_pack, outer_unpack = outer
    inner_pack, inner_unpack = inner

    def pack(x):
        return inner_pack(outer_pack(x))

    def unpack(packed):
        return outer_unpack(inner_unpack(packed))
