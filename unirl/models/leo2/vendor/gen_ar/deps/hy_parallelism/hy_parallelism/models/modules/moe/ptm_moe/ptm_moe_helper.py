...
def assert_grouped_gemm_is_available():
    assert grouped_gemm_is_available(), (
        ...
        "`pip install git+https://github.com/fanshiqing/grouped_gemm@v1.0` (DEPRECATED).\n"
        "Or run: "
        "cd /apdcephfs_cq8/share_2938211/kevinkhwu/git/AngelPTM/grouped_gemm && python3 setup.py install"
    )
...
class GroupedMLP(torch.nn.Module):
    ...
    def reset_parameters(self):
        ...
        init_param(self.group_h_to_4h)
        init_param(self.group_4h_to_h)
    ...
    def forward(self, permuted_local_hidden_states, tokens_per_expert):
        ...
        # TODO(kevinkhwu)
        if self.config.use_grouped_gemm and permuted_local_hidden_states.nelement() != 0:
...
class PTMHunYuanMLP(torch.nn.Module):
    def __init__(self, config: MOEConfig, is_shared_mlp=False):
        super().__init__()
        self.config = config
