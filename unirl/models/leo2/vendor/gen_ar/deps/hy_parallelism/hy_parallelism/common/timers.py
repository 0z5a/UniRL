...
class Timer(TimerBase):
    ...
    def start(self, barrier=False):
        ...
        from hy_parallelism.tools.profiling import range_push
    ...
    def stop(self, barrier=False):
        ...
        if not self._started:
            logger.warning(f'timer {self.name} is not started.')
            return
        # assert self._started, 'timer is not started'
        if barrier:
            torch.distributed.barrier(group=self._barrier_group)
        torch.cuda.synchronize()
        from hy_parallelism.tools.profiling import range_pop
        range_pop()
...
class Timers:
    ...
    def log(
        ...
        if rank is None:
            rank = self.get_world_size() - 1
        if rank == self.get_rank() and output_string is not None:
...
def _set_timers():
    ...
    _GLOBAL_TIMERS = Timers(log_level=2, log_option='all')
...
def get_timers() -> Timers:
    ...
    if _GLOBAL_TIMERS is None:
        _set_timers()
    assert isinstance(_GLOBAL_TIMERS, Timers), f'{_GLOBAL_TIMERS=}'
