...
def zeropower_via_newtonschulz5(G, steps = 5):
    ...
    if isinstance(G, DTensor):
        ...
        G = G.full_tensor()
        #G = G.redistribute(placements=[Replicate()] * device_mesh.ndim, async_op=True)
    else:
        device_mesh = None
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                # apply update
                #print("count:",count, "p type:", type(p), "u type:", type(u), "p shape", p.shape, "u shape", u.shape, "lr:", -adjusted_lr)
...
def get_muon_for_engine(params, lr=1e-3,  weight_decay=0.1, momentum=0.95, adamw_betas=(0.95, 0.95), adamw_eps=1e-8):
    muon_params = [
        p
        for p in params
        if p.ndim >= 2
