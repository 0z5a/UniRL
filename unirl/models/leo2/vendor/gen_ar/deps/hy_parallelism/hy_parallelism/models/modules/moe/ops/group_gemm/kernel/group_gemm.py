...
from hy_parallelism.models.modules.moe.ops.group_gemm.utils.pretuned import algo_key_scaled, pretuned
from hy_parallelism.models.modules.moe.ops.group_gemm.kernel.triton_utils.activation import (
    ActivationType,
    activation_fwd,
)
from hy_parallelism.models.modules.moe.ops.group_gemm.kernel.triton_utils.memory import (
...
from hy_parallelism.models.modules.moe.ops.group_gemm.kernel.triton_utils.utils import (
    get_pid_mn,
    make_blocked,
)
...
def _get_cuda_autotune_config():
    return [
        triton.Config({'BLOCK_M': 128, 'BLOCK_N': 32, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=4,
                      num_warps=4),
    ]
...
#         triton.Config({'BLOCK_M': 128, 'BLOCK_N': 256, 'BLOCK_K': 64, 'GROUP': 8}, num_stages=3,
#                       num_warps=8),
#         triton.Config({'BLOCK_M': 64, 'BLOCK_N': 256, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=4,
#                       num_warps=4),
#         triton.Config({'BLOCK_M': 128, 'BLOCK_N': 128, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=4,
#                       num_warps=4),
#         triton.Config({'BLOCK_M': 128, 'BLOCK_N': 64, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=4,
#                       num_warps=4),
#         triton.Config({'BLOCK_M': 64, 'BLOCK_N': 128, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=4,
#                       num_warps=4),
#         triton.Config({'BLOCK_M': 128, 'BLOCK_N': 32, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=4,
#                       num_warps=4),
#         triton.Config({'BLOCK_M': 64, 'BLOCK_N': 32, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=5,
#                       num_warps=2),
#         triton.Config({'BLOCK_M': 32, 'BLOCK_N': 64, 'BLOCK_K': 32, 'GROUP': 8}, num_stages=5,
#                       num_warps=2),
#     ]
#     return configs
...
def group_gemm_same_nk_kernel(
    ...
    offs_am = offs_m_i64 % m_size.to(tl.int64)
    offs_bn = offs_n % N
    ...
    c_ptrs = c_ptr + N * offs_m_i64[:, None] + offs_n[None, :]
    ...
    if HAS_ACTIVATION:
        ...
        if SAVE_ACTIVATION:
            store_with_pred_2d(
                act_ptr + gtid_start * N + N * offs_m_i64[:, None] + offs_n[None, :],
                c,
                False,
                N_ALIGNED,
                offs_m[:, None] < m_size,
...
def group_gemm_same_nk(
    ...
    Keyword arguments:
    a -- lhs matrixs to be matrix multiplied   transpose_a=False: [tokens, h], else: [h, tokens]
    b -- rhs matrixs to be matrix multiplied   transpose_b=False: [num_experts, h, h], else: [num_experts, h, h]
