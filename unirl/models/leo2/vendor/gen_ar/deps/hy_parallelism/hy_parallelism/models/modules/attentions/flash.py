...
class FlashAttnMaskInfo:
    ...
    def __init__(
        ...
        pack: bool,
        used_seqlens_in_batch: torch.Tensor | None = None,
    ):
        self.indices = indices
        self.cu_seqlens = cu_seqlens
        self.max_seqlen_in_batch = max_seqlen_in_batch
        self.pack = pack
        self.used_seqlens_in_batch = used_seqlens_in_batch

    @classmethod
    def from_attention_mask(cls, attention_mask: torch.Tensor, pack: bool = False):
        # flash_attn.bert_padding.unpad_input / flash_attn.bert_padding.unpad_input_for_concatenated_sequences
        if pack:
            length = attention_mask.sum(dim=-1)
            seqlen = attention_mask.size(-1)
            attention_mask_2d = (
                torch.arange(seqlen, device=length.device, dtype=length.dtype).expand(
                    len(length), seqlen
                )
                < length.unsqueeze(1)
            )
            real_indices_idx = torch.nonzero(attention_mask.flatten(), as_tuple=False).flatten()
            seqlens_in_batch = attention_mask.flatten()[real_indices_idx]
        ...
        used_seqlens_in_batch = seqlens_in_batch
        ...
        return cls(
            ...
            max_seqlen_in_batch=max_seqlen_in_batch,
            pack=False,
            used_seqlens_in_batch=used_seqlens_in_batch,
        )

    def unpad(self, hidden_states: torch.Tensor):
        # flash_attn.bert_padding.unpad_input / flash_attn.bert_padding.unpad_input_for_concatenated_sequences
        hidden_states_unpad = index_first_axis(
            rearrange(hidden_states, "b s ... -> (b s) ..."), self.indices
        )
        if self.pack:
            return (
                hidden_states_unpad,
        ...
        return (
            ...
            self.cu_seqlens,
            self.max_seqlen_in_batch,
            self.used_seqlens_in_batch,
        )

    def __repr__(self) -> str:
        return f"FlashAttnMaskInfo(indices={self.indices}, cu_seqlens={self.cu_seqlens}, max_seqlen_in_batch={self.max_seqlen_in_batch}, pack={self.pack}, used_seqlens_in_batch={self.used_seqlens_in_batch})"
