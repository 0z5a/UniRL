...
from ..utils import arrow_file_to_shadow
...
def source_names_to_names(source_names_):
    ...
    Examples:
    `/mnt/share/cq10/jarvizhang/workspace/IndexKits/test/test.arrow@@x2` means repeat the
    arrow file `test.arrow` 2 times.
    """
    names = []
    exclude_count = 0
    for data_name in source_names_:
        ...
        for name in glob(data_name):
            for e in exclude:
                ...
                row_names.append((name, repeat_times))
                cur_count += 1
        print(f"Found {cur_count} arrow files in {data_name}.")

        # Sort names in the current data_name by name.
        row_names = list(sorted(row_names, key=lambda x: x[0]))
...
def get_or_create_arrow_name_list(source_names=None):
    assert source_names is not None, "`source_names` is required."
...
def to_numeric(x, default_val=0.0):
    ...
    if isinstance(x, str):
        try:
            ...
            print(f"Warning: Cannot convert string to float: {x}. Using default value: {default_val}")
            x = default_val
    if isinstance(x, (np.ndarray, list)):
        return to_numeric(x[0], default_val=default_val)

    # Now we assume x is a numeric type or can be converted to float
    if np.isnan(x):
        return default_val
    elif isinstance(x, (float, int, np.integer)):
        pass
    else:
        try:
            x = float(x)
        except Exception as e:
            print(f"Warning: Cannot convert {type(x)} to float: {x}. Using default value: {default_val}")
            x = default_val
    return x
...
class ColumnFilter(Operator):
    ...
    def __init__(self, config):
        ...
        self.repr = self.data_type
        # Support shadow file
        self.shadow_suffix = config.get('shadow_suffix', None)

    def maybe_shadow(self, arrow_file, table):
        if self.shadow_suffix is not None:
            # Read shadow table
            arrow_file = arrow_file_to_shadow(arrow_file, self.shadow_suffix, assert_exist=True)
            table = get_table(arrow_file)
        return arrow_file, table
...
class IntFilter(NumericFilter):
    def __call__(self, arrow_file, table):
        arrow_file, table = self.maybe_shadow(arrow_file, table)
        ...
        try:
            ...
            print(f"Error while processing {arrow_file} for column '{self.column_name}'")
            raise e
...
class StrFilter(ColumnFilter):
    def __call__(self, arrow_file, table):
        ...
        if not success:
            return None
        column = table[self.column_name].to_pandas().fillna(self.default)
        if self.action == 'eq':
...
class BytesFilter(ColumnFilter):
    def __call__(self, arrow_file, table):
        arrow_file, table = self.maybe_shadow(arrow_file, table)
        success = self.check_exists(arrow_file, table)
        if not success:
...
def get_column_filter(parent, config):
    ...
    elif config['type'] == 'bytes':
        valid_actions = ColumnFilter.bytes_actions
    else:
    ...
    if config['type'] == 'int':
        ...
        return True, "", BytesFilter(config)
    else:
...
class FilterCompose(object):
    def __init__(self, column_filter_list, md5_filter_list, md5_col="md5"):
    ...
    def __call__(self, arrow_file, table):
            ...
            if isinstance(filter_, tuple):
                ...
                if op == 'logical_or':
                    ...
                    if sub_mask is not None:
                        name = '|'.join([f.repr for f in filter_list])
                        stats.update({
                ...
                if current_mask is not None:
                    stats.update({
                        filter_.repr: length - sum(current_mask)
                    })
                    if mask is None:
                        mask = current_mask
                    else:
        ...
        for filter_ in self.md5_filter_list:
            if not filter_.applicable(arrow_file):
                continue
            if self.md5_col not in table.column_names:
                print(f"Warning: Column '{self.md5_col}' not found in {arrow_file}.")

            md5s = table[self.md5_col].to_pandas()
...
class RepeaterCompose(object):
    def __init__(self, repeater_list, md5_col="md5"):
...
class DatasetConfig(object):
    def __init__(self, work_dir, config_file):
        ...
        self.assert_unknown_args()

        # Extra arguments
        self.remove_md5_dup = self.data.get('remove_md5_dup', False)
        self.md5_col = self.data.get('md5_col', 'md5')
        ...
        self.ceph_base = self.parse_ceph_base()

    def assert_unknown_args(self):
        unknown_args = set(self.data.keys()) - {
            'source', 'filter', 'repeater', 'ceph_base', 'remove_md5_dup', 'md5_col',
        }
    ...
    def parse_filter(self):
        ...
        if column_filter_list or md5_filter_list:
            composed_filter = FilterCompose(column_filter_list, md5_filter_list, md5_col=self.md5_col)
        else:
    ...
    def parse_repeater(self, enable_arrow_repeat=False):
        ...
        if repeater_list or enable_arrow_repeat:
            composed_repeater = RepeaterCompose(repeater_list, md5_col=self.md5_col)
        else:
    ...
    def parse_ceph_base(self):
        if 'ceph_base' in self.data:
            ceph_base = self.data['ceph_base']
            assert isinstance(ceph_base, dict), "ceph_base should be a dict."
        else:
            ceph_base = {}
        return ceph_base
