...
Use `--compare-results` with the four generated JSON files. The comparison
fails non-zero for incompatible workloads, different shared GPUs, score
parity failures, or request failures. Generated run data is intentionally not
committed.
