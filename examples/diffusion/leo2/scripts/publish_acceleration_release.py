...
def _metric_row(
    ...
    *,
    ...
    for name, value in quality.items():
        if (
            name.startswith("vbench_")
            or name.startswith("videoscore2_")
            or name in {"latent_mse", "latent_rmse"}
        ):
            row[name] = value
    return row
...
def _atomic_write(path: Path, payload: dict[str, Any], *, replace: bool) -> None:
...
def main() -> None:
    ...
    payload = build(args)
    _atomic_write(args.output, payload, replace=False)
    if args.current is not None:
        _atomic_write(args.current, payload, replace=True)
    print(args.output)
